...
Выбор `mode` — производная от `urgency`:
- `normal` → `'normal'`
- `low` → `'tenths'`
- `critical` → `'hundredths'`

Для **неактивной стороны** всегда `'normal'`, даже если её `urgency` уже `low`. Иначе у обоих игроков скачут доли, что отвлекает; lichess делает то же самое.
