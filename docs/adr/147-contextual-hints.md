...
- Целевой объём аналитики — 50–150K событий/день (см. §2.3). На таком объёме отдельный аналитический стек (Clickhouse, Kafka, dedicated warehouse) технически избыточен: вся нагрузка укладывается в одну PostgreSQL-инстанцию с правильными индексами и retention. Любая новая инфраструктурная единица (отдельный сервис, СУБД, очередь) добавляет в эксплуатацию ≥4 часа/мес на патчи/мониторинг/восстановление и стоит ≥$40/мес на хостинг в минимальной конфигурации — это плата, не окупающаяся при текущем объёме.
...
| **PostHog (self-hosted) для аналитики + наш HintsEngine** | Богатые аналитические возможности, готовые SDK, voronka/A-B/cohorts «из коробки» | Минимальная prod-инсталляция — 3 контейнера (web, worker, plugin-server) + Clickhouse + Kafka/Redpanda + zookeeper/etcd. На EC2/RDS — ≥$160/мес ресурсов (по AWS pricelist 2026: t3.medium + db.t3.small + Clickhouse storage 100 GB gp3). Обновления PostHog требуют миграций Clickhouse и проверки совместимости плагинов — оценочно 4–6 ч/мес поддержки. Учить новый DSL запросов (HogQL). | Отвергнуто. Для 50–150K событий/день (1.5–5M/мес) Postgres-таблица с двумя композитными индексами даёт p95 запроса <20 мс и стоит 0 дополнительных ресурсов поверх существующего RDS. Богатые возможности PostHog (funnels, cohorts) для задачи «триггер подсказки по простому условию» не требуются. Если в будущем появится отдельная задача продуктовой аналитики (а не триггеров подсказок) — пересмотреть. |
