...
- `STABILIZED_CONSECUTIVE_PLIES = 2` — минимум одна пара ходов в стабильности (1 полуход = только наш или только соперник, не информативно).
...
- `+` (check), `#` (mate) — продлеваем ещё на 1 ход (соперник вынужден отвечать на угрозу).
...
- §4.2 «Maia-trap»: Maia top-1 как side-variation, только если пользователь сам его не сыграл, и Maia top-1 классифицируется как `mistake`/`blunder`.

Этого мало. Самый частый use-case — «соперник в этой позиции часто играет X, как его наказать» — не покрыт.

### 4.2. Новое: Maia-альтернатива в линиях

При построении любой линии (через §3 алгоритм) для каждого полухода соперника (нечётные индексы при счёте с 1) проверяем:

```ts
// На позиции перед ответом соперника:
const sfBestOpp = engineGetBestLine(fen).bestUci;
const maiaPolicy = maiaPredict(fen, elo).policy;
const maiaTopOpp = topProbabilityMove(maiaPolicy);

if (
  maiaTopOpp !== sfBestOpp
  && maiaPolicy[maiaTopOpp] >= MAIA_ALT_MIN_PROB  // 0.20
  && classifyMove({ wdlBefore, wdlAfter: wdlAfterMaiaTop }) !== 'best'
) {
  // Добавляем sub-variation: maiaTopOpp + наш SF-best ответ + продолжение
  // через тот же buildStabilizedLine (рекурсивно), но с пониженным cap.
}
```
...
### 4.4. Не добавляем для собственного ответа (heuristic suppression)

Если ход соперника — это `mainLineSfBest` и Maia top-1 соперника совпадает с SF-best — `maiaAlt` варианта не добавляем (тождественная линия). Это уже обработано через `maiaTopOpp !== sfBestOpp`.

### 4.5. На ходах игрока — нужно ли?

Спорный вопрос. Когда игрок сыграл хорошо, но Maia top-1 ≠ sfBest — показать ли «вот что обычно играют люди этого ELO»?

- Pro: пользователь видит более полную картину человеческой игры.
- Con: дерево становится зашумлённым; на каждом good-ходе добавляется sub-variation.

**Дефолт MVP — НЕТ.** Maia-альтернативы добавляем **только** на ходах соперника (4.2), на ходах игрока — только в трёх случаях: §4.1 ADR-100 (старое «как надо было»), §4.2 ADR-100 (старая «Maia-trap» когда игрок не сыграл Maia top-1 и он реально плох), и третий случай — see §6 open question.
...
| `MAX_NESTED_DEPTH` | 4 | Sub-sub-variation допустим (например, «как наказать» → «но если соперник пойдёт ещё неожиданно»), но 5-й уровень — уже неподьёмно. |
...
## 6. Открытый вопрос (единственный критичный)

**Maia-альтернатива на ходах игрока (не соперника), даже когда игрок сыграл хорошо.**

Дефолт ADR — НЕ добавляем (§4.5). Аргумент: дерево вариантов и так растёт за счёт ответов соперника, добавление ещё и «человеческой альтернативы игроку» на каждом good-ходе перегружает дерево.

Альтернатива — добавляем по тому же правилу что §4.2 (но без требования «не сыгран» — игрок мог сыграть и хорошо, и плохо). Это даёт максимально полное покрытие, но шумит.

Если пользователь скажет «да, добавляем» — расширяем условие в §4.2 на «не только на ходах соперника, но и на ходах игрока». Это +1 строчка в коде; основное — согласовать UX-приоритет.

**Дефолт: НЕТ (как в ADR).** Подтвердить.
...
На каждом полуходе соперника (40 шт для партии 80 полуходов) — 1 Maia inference уже есть (от ADR-100). При совпадении maiaTop=sfBest — variation не добавляем (no extra SF). При различии — нужен `wdlAfterMaiaTop` (1 доп. SF-run). Это ~+10-15 доп. SF-runs на партию, незначительно.

Sub-variation от Maia-альтернативы (длина 1..4 полухода) — те же ~3.5 SF-runs в среднем. Sub-variations добавляются примерно к 30-40% полуходов соперника = ~12-16 sub-variations × 3.5 = ~50 SF-runs. С кэшем — 25-30. **~5-10 с доп.**

### 7.3. Итоговый бюджет

Worst-case ~+50% времени поверх ADR-100 §7. p50 на партию 80 полуходов = **~60-100 с** (вместо ~25-40 с в ADR-100). На стандартной 40-полуходной — ~30-50 с.
...
3. Доработка `apps/web/src/lib/review/buildAnnotations.ts`:
   ...
   - Новое §4.2 (4.2): для каждого хода соперника в построенной линии — проверка `maiaTopOpp !== sfBestOpp && classify(maiaAlt) !== 'best'` → добавить sub-variation. Реализация — обходим линию после её построения и обогащаем variations'ами.
...
5. Тесты (Vitest):
   ...
   - `buildAnnotations.test.ts` дополнить:
     ...
     - Maia-альтернатива добавляется на ответе соперника при разных classification.
...
- На полуходах соперника (где Maia top-1 ≠ SF best и классифицируется не как best) — добавляются sub-variations с Maia-альтернативой + наш ответ.
- Лимиты §5 соблюдаются (max nested depth, max variations per node, soft cap per move).
- p50 анализа партии (40 полуходов) укладывается в ~30-50 с, p95 (80 полуходов) — в ~60-100 с.
...
- Maia-альтернатива на good-ходах игрока (см. §6 open question, дефолт — НЕТ).
...
Maia top-1 теперь добавляется как sub-variation **на ходах соперника** в любой линии — когда maiaTop ≠ sfBest и не классифицируется как `best` (порог prob = 0.20). Sub-variation тоже строится через тот же алгоритм стабилизации с cap = 4.

Лимиты: max 4 уровня вложенности, max 12 variations на ход (soft cap с приоритетом по Maia prob). Стоимость +50% к ADR-100 §7 бюджету; p50 на 80-полуходную партию ~60-100 с — приемлемо.

Один открытый вопрос — Maia-альтернатива на good-ходах игрока (дефолт: НЕТ, ожидаем подтверждение).
