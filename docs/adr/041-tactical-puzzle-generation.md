...
Двухэтапная проверка по позиции после хода, который анализируем. На каждом ходу партии (start ≥ ply 20):

1. **Blunder detection** на сделанном ходе:
   - `evalDrop = |bestScore − madeMoveScore|` (cp; для матовых оценок — конвертация в условный cp 1000+matedist).
   - Принять, если `evalDrop ≥ 200cp`. Иначе — это не пропуск тактики, идём дальше.

2. **Uniqueness** в позиции «после blunder» (это и будет начальная позиция puzzle):
   - `spread = |bestScore − secondScore|` в MultiPV-результате стороны на ходу.
   - Принять, если `spread ≥ 150cp`. Иначе несколько хороших ходов — обычная позиция, не тренировочный puzzle.
...
После того как позиция прошла проверки 2.2.1 и 2.2.2 — это **позиция после blunder**, ход за стороной, у которой был лучший ответ. Линия строится итеративно:

- На каждом шаге `i` (нечётный i = ход «решающего» — нашего, чётный = ответ соперника):
  1. Stockfish MultiPV=2 на текущей позиции, depth=20.
  2. Если `i` — наш ход: применить `bestMove`, требовать `spread ≥ 150cp` между лучшим и вторым (иначе строго не уникально → стоп, обрезаем линию). Сравнение через текущий side-score.
  3. Если `i` — ход соперника: если у соперника **только один** легальный ход — применить его (форсированно). Иначе MultiPV: применяем «защищающий» лучший ход; принимаем только если все его легальные ходы дают одинаковую (или близкую, ±50cp) итоговую оценку — тогда «вилка соперника» нерелевантна, любая защита проигрывает. Иначе — стоп.
- Стоп-условия:
  - длина ≥ 6 полуходов;
...
- **Рекапчер на старте**: если первый ход линии — взятие на той же клетке, где было предыдущее взятие в партии (т.е. простой recapture после торгов), и spread ≥ 150 даёт это «бесплатно» — drop. Реализация: смотрим `madeMove.captured` в партии и сравниваем целевую клетку.
...
- `gap`: `spread` в cp (для UI «насколько чисто решение»);
...
- `sourceMetadata` (JSON): `{ "evalDrop": …, "lineLength": …, "engine": "stockfish-XX", "engineVersion": "…", "generatedAt": "…" }`.
...
gapAdj = gap > 400 ? −100 : gap < 200 ? +100 : 0
rating = clamp(600, 2800, avgPlayerRating + lengthAdj + gapAdj)
```

Это формула из KS-1408 без изменений; Glicko на attempts далее скорректирует.
...
- `evalDrop ≥ 200cp` (есть тактика).
- `spread ≥ 150cp` на ходе решения и на каждом нашем последующем ходе.
- `2 ≤ lineLength ≤ 6`.
...
| Spread | 200–800cp | < 150 (несколько решений), > 800 (очевидно) |
