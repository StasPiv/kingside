...
**Дата:** 2026-05-05 (правка 2026-05-05: §2 переписан под реальный алгоритм lichess-puzzler `analyze_position` / `cook_advantage` / `cook_mate` — триггер «переход в выигранную позицию», spread `0.7 WC`, нечётная длина ≥ 5. **Pair_limit сознательно урезан** до `depth=22/time=10s/nodes=10M` ради CPU-бюджета — критерии отбора lichess-овые, глубина движка наша. Обоснование в §2.1, бюджет в §3.4)
**Статус:** Предложено
**Задача:** KS-2430
**Связанные:**
...
Лимиты сознательно урезаны относительно lichess-puzzler. У lichess `pair_limit = depth=50, time=30s, nodes=25M` — это даёт ~10–18s/позиция, что при наших ресурсах превращает 1000 партий в 25+ часов. Для тактического пазла такая глубина избыточна: `depth=22` уже надёжно покрывает форсированные линии до 8–10 полуходов (наш максимум — 9). Урезаем глубину, но **сохраняем lichess-критерии отбора без изменений** (`ΔWC > 0.6` для триггера, `> 0.7` для spread в линии — они не зависят от глубины движка).

| Параметр | Значение | Lichess-эталон | Почему отличается |
|---|---|---|---|
| `pair_limit` (advantage) | `depth=22, time=10s, nodes=10_000_000` | `depth=50, time=30s, nodes=25M` | См. выше; покрывает все наши длины линии. |
| Mate-defense limit | `depth=15, time=10s, nodes=8_000_000` | то же | Без изменений — у lichess уже короткий лимит. |
| MultiPV | 2 | 2 | Без изменений. |
| `mate_soon` | `Mate(15)` | `Mate(15)` | Без изменений. |

Stockfish вызывается с soft-лимитами (любое из условий — depth/time/nodes — гасит расчёт). Реальное среднее ожидаем в районе 3–5 сек/позиция на современном Stockfish 17 (~1.5M nodes/sec на vCPU); это допущение проверяется §5.2 первым прогоном.

Сразу после получения cp/mate из Stockfish — конвертация в WC по формуле §2.0; дальнейшие сравнения идут в WC.

**Цена компромисса:** часть пазлов с пограничным spread'ом (где лучший ход начинает доминировать только с `depth ≥ 30`) не пройдут `is_valid_attack` на `depth=22` и отсеются. Это **дополнительный отсев в сторону чистоты**, не источник ложных пазлов: пороги `0.7 WC` остаются жёсткими, просто их сложнее пройти на меньшей глубине. Если §5.2 покажет, что мы теряем существенный класс правильных пазлов — пересматриваем `depth` отдельным апдейтом.
...
Под наш урезанный `pair_limit` (`depth=22, time=10s, nodes=10M`). Расчёт строится на трёх параметрах, каждый из которых явно отмечен как факт или допущение:

| Параметр | Значение | Источник |
|---|---|---|
| Среднее время на позицию | 4 сек | **Допущение.** Stockfish 17 ≈ 1.5M nodes/sec на vCPU; при `nodes=10M` это ~6–7s, но большинство тактических позиций сходится раньше по `depth=22`. Проверяется §5.2 первым прогоном. |
| Позиций на партию | 50 | **Оценка.** TWIC-партия ≈ 60–80 ply, минус `ply ≥ 20` (§2.3) → 40–60 анализируемых позиций. Берём 50 как медиану. |
| Эффективная параллель | 4× | **Оценка.** `STOCKFISH_POOL_SIZE = 5` на 4 vCPU; реальное ускорение из-за конкуренции ~×4, не ×5. |
| Доп. время на построение линий | +50 сек/партия | **Оценка.** Триггер срабатывает 1–2 раза за партию, `cook_advantage` гоняет `pair_limit` ещё ~10 раз (5 наших + 5 ответов соперника) на каждой сработке. |

`T(партия) = (50 × 4) / 4 + 50 = 100 сек ≈ 1.7 минуты` на 4 vCPU.

| Объём | На 1 vCPU | На 4 vCPU (1 task pool=5) |
|---|---|---|
| 100 партий | ~7 ч | ~1.7 ч |
| 1000 партий | ~70 ч | ~17 ч |
| 10000 партий | ~30 суток | ~7 суток |

Daily-incremental 500 партий — ~8.5 часов на 4 vCPU, помещается в night-window с запасом. MVP-batch на 100 партий для §5.2 — менее 2 часов.

**Что меняется, если допущения неверны:**
- если среднее на позицию = 6s (а не 4) → 100 партий ≈ 2.5 часа (всё ещё ок);
- если среднее = 8s → 100 партий ≈ 3.3 часа, daily-incremental не помещается → снижаем `nodes` до 5M в следующем апдейте ADR;
- если позиций на партию 60 (не 50) → +20% времени, не критично.

Эмпирические значения собирает [qa] smoke-тест (10 партий) в §6 этап 1. После него §3.4 обновляется реальными цифрами.
