...
Статус: **Rejected** (KS-3572, 2026-06-02).
Дата: 2026-06-02.
...
Решением пользователя (KS-3572, 2026-06-02) ADR отклонён. Финальное требование: AI-ассистент **остаётся на личном ноутбуке пользователя** через текущий `webhook-server.py`. OAuth-credentials Claude Pro/Max в нашу инфраструктуру (AWS Secrets Manager, ECS Fargate, контейнеры в нашей зоне) **не выносятся**.

Следствия:
- Сервис `apps/ai-runtime`, описанный в §5, **не создаётся**.
- Зависимые задачи декомпозиции из §8 (Этапы A-F) **отменены**. KS-3573 (smoke OAuth-credentials в Docker) — отменён, backend останавливает работу.
- Текущая инфраструктура (webhook на VPS Kamatera + claude CLI на ноутбуке пользователя) **остаётся в силе**. Проблемы из KS-3321 (45-секундный таймаут, cold-start `--resume`) решаются в зоне пользователя — правкой `webhook-server.py`, а не миграцией runtime в нашу инфру.
- Если в будущем требование изменится — этот ADR можно использовать как стартовую точку для нового пересмотра, но содержимое ниже считать **исторической справкой**, не действующим решением.

Текст ниже сохранён как есть на момент принятия решения об отклонении.
