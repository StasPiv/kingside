...
| 5 | `find-mate-in-one-square` | Клетка, куда поставить мат в один ход | `moves({verbose:true})` → играем каждый, проверяем `isCheckmate()`. В позиции должен быть **ровно один** ход, ставящий мат, иначе drop. Дополнительно — **отбрасываем позиции, где две разные фигуры дают мат на разные `to`-клетки** (KS-2223 §8.3) | `square` (to-клетка) **строго**; fallback на `move` отменён в KS-2223 §8.3 | сторона на ходу = матующая |
