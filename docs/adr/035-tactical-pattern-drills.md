...
| 4 | `find-fork` | Ход, **создающий новую вилку** у соперника. «Вилка» = одна наша фигура одновременно атакует ≥2 ценных вражеских (n/b/r/q/k); пешки в качестве цели исключены, король всегда «ценен» (см. §2.2.1 (g) терминология). Существующие вилки в исходной позиции — **не** засчитываются: нужна именно новая, появившаяся после хода. Термин «двойной удар» (double attack) шире вилки и зарезервирован под расширение в v2 | Snapshot `forksBefore` = множество клеток наших фигур, которые уже атакуют ≥2 ценных вражеских. Для каждого легального `m`: apply `m` → собрать `forksAfter` тем же правилом → undo. Кандидат, если `forksAfter \ forksBefore ≠ ∅` (появилась новая forker-клетка). Strict-uniqueness: ровно 1 такой ход (по паре `(from, to)`), иначе drop. KS-2399 — переход с прежнего статического `shape='square'` на `shape='move'` по образцу `find-undefended-attack` (KS-2387): переводит тип из «распознать паттерн» в «совершить тактическое действие» и автоматически выравнивает дисбаланс по piece-type forker'а (статика смещалась на ферзя, динамика чаще даёт коня/лёгкую фигуру) | `move` (from + to). Promotion в v1 отбрасывается | важна |
...
- `find-pin` и `find-fork` могут пересекаться: после KS-2399 (`find-fork` динамический) пересечение читается как «ход одновременно создаёт связку и вилку». Indexer отбрасывает такие позиции по drill-набору, чтобы не путать UI (один и тот же ход не должен попасть в обе выборки).
...
| **Вилка** | **Fork** | Одна наша фигура F одновременно атакует ≥2 ценных вражеских (n/b/r/q/k); пешки как цель исключены, король «ценен» как любая ценная фигура. Защитники целей **не** учитываются — даже защищённая фигура в вилке проблематична: одну из двух заберём бесплатно. Источник истины: `forker(F) = | attacks_from(F) ∩ {valuable enemy pieces} | ≥ 2`. | `find-fork` (тип 4) — создать ход, дающий новую вилку |
...
- `find-undefended-attack` (динамика, ответ — наш атакующий ход): висящих в позиции **может не быть** (или они уже учтены через `threatsBefore`), нужно ходом создать **новую**. Способы создания: прямой удар, discovered, interference, отвлечение/устранение защитника.
- `find-fork` (динамика, ответ — наш атакующий ход): фокус смещён с **одной цели + защитники** (висящая) на **≥2 цели одновременно** (вилка). Защитники не учитываются. Способы создания вилки: ход коня на forker-клетку (классический жокей-форк), вскрытие линии sliding-фигуры с одновременной атакой двух, discovered-fork (уйти с линии так, чтобы открывшаяся атака + остаточная атака покрыли две цели). Семантическое отличие от undefended-attack: в вилке ≥2 цели, в undefended-attack — одна цель + проверка отсутствия защитников.
...
| find-fork | «Какой ход создаёт у соперника новую вилку?» | «Which move creates a new fork for the opponent?» |
...
Полный набор i18n-ключей `find-fork` (RU + EN, KS-2399) — фиксируется здесь как окончательный для v1 и подлежит переносу в `apps/web/src/i18n/locales/{ru,en}/translation.json`:

| ключ | RU | EN |
|---|---|---|
| `drills.types.findFork` | «Создать вилку» | «Create a fork» |
| `drills.instructions.findFork` | «Какой ход создаёт у соперника новую вилку (одновременную атаку на две ценные фигуры)?» | «Which move creates a new fork (simultaneous attack on two valuable pieces) for the opponent?» |
| `drills.instructions.findForkWhite` | «Какой ход белых создаёт у чёрных новую вилку?» | «Which White move creates a new fork for Black?» |
| `drills.instructions.findForkBlack` | «Какой ход чёрных создаёт у белых новую вилку?» | «Which Black move creates a new fork for White?» |
| `drills.typeDescriptions.findFork` | «Найдите ход, после которого ваша фигура одновременно нападает на две ценные фигуры соперника (конь, слон, ладья, ферзь, король). Такая атака называется вилкой.» | «Find a move after which your piece simultaneously attacks two valuable opponent pieces (knight, bishop, rook, queen, king). Such an attack is called a fork.» |

Прежняя формулировка `«Какая фигура делает вилку?»` (KS-2227 / shape='square') заменена на динамический вариант после KS-2399 — типу присвоен `shape='move'`. Обоснование выбора shape — §2.1 строка 4 + §2.2.1 (g) различие связанных типов; обоснование термина **вилка / fork** — §2.2.1 (g) терминология.

Остальные шесть типов — формулировки в таблице выше; правила онбординга и финальные тексты для них могут уточняться chess-expert (см. KS-DRILL-METHOD ниже).
...
- `find-fork` — нужно `from + to` (после KS-2399 тип stал shape='move'). Двух-кликом аналогично `find-undefended-attack`. **Никакого drag-only.**
...
| R4 | `find-fork` / `find-pin` могут давать неоднозначные ответы. Для `find-pin` — несколько связок в позиции; для `find-fork` после KS-2399 — несколько ходов, создающих новую вилку | backend (E2) | Предикат отбрасывает позиции с >1 валидным ответом. Для `find-fork` strict-uniqueness считается по полной паре `(from, to)` хода-форкера (по аналогии с `find-undefended-attack`) |
