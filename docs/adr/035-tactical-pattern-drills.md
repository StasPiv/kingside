...
|---|---|---|---|---|---|
| 1 | `find-hanging-piece` | Незащищённая фигура соперника, на которую можно безнаказанно напасть (или есть атакующий с нулевой защитой) | Для каждой `enemy piece` на `sq`: `attackers(sq, ourColor)` ≥ 1 **и** `attackers(sq, enemyColor)` = 0 (либо SEE > 0). Король исключён | `square` (одна правильная клетка; если их несколько — любая засчитывается, но в задаче должна быть **ровно одна** для однозначности) | важна |
| 2 | `find-all-checks` | Все клетки **назначения** хода с шахом для стороны на ходу | `Chess.moves({verbose:true}).filter(m => m.san.includes('+'))` → уникальные `to`-клетки | `squares[]` (нужно отметить все, score = TP/(TP+FP+FN)) | важна |
| 3 | `find-pin` | Связанная фигура (любого цвета). По умолчанию — связки на короля (абсолютные). В v2 добавим относительные | Для каждой фигуры P цвета C на `sq`: убрать P → если `kingSq(C)` теперь под боем фигуры дальнобойного типа, лежащей на той же линии «attacker → P → king», и эта фигура атакует через `sq` — связка | `square` (P) | не важна (можно показывать с обеих сторон) |
| 4 | `find-fork` | Двойной удар: один ход атакует ≥2 ценных фигур противника одновременно. Версия MVP — найти **готовую** вилку (фигура уже атакует двух) | Для каждой фигуры `F` стороны на ходу: `attacks_from(F)` ∩ `enemy pieces of value ≥ minor` ≥ 2. Король всегда «ценен» | `square` (фигура, делающая вилку) | важна |
| 5 | `find-mate-in-one-square` | Клетка, куда поставить мат в один ход | `moves({verbose:true})` → играем каждый, проверяем `isCheckmate()`. В позиции должен быть **ровно один** ход, ставящий мат, иначе drop | `square` (to-клетка) либо `move` (если задача неоднозначна по фигуре) | сторона на ходу = матующая |
| 6 | `find-loose-piece` (отдельно от `hanging`) | Фигура противника, у которой число защитников = 0 (даже если нападающих нет). Тренирует «обзор» доски, а не расчёт обмена | `attackers(sq, enemyColor)` = 0 для каждой фигуры противника. Ровно одна в позиции | `square` | не важна |
...
| 8 | `find-undefended-attack` | Найти ход, который **нападает** на фигуру, не имеющую защитников (создание угрозы, не съедание) | Для каждого хода `m`: после `m` существует `enemy_piece` под боем, у которой `attackers(_, enemyColor)` = 0. Ровно один такой ход в задаче | `move` (from + to) | важна |
...
Окончательные формулировки — задача chess-expert (KS-DRILL-METHOD ниже).
...
| R6 | Локализация инструкций — chess-expert + chess.js notation (фигуры в RU «Ф/Л/К/С/К/п»). На UI показываем символы, не буквы | chess-expert (E0) | Использовать unicode фигур (♛♜...) вместо букв |
