...
| 2 | `find-all-checks` | Все клетки **назначения** хода с шахом для стороны на ходу | `Chess.moves({verbose:true}).filter(m => m.san.includes('+'))` → уникальные `to`-клетки. **Инварианты (KS-2313):** `2 ≤ |unique to| ≤ 7` — позиции с одиночным шахом или > 7 шахов отбрасываются. Дополнительно: позиции, в которых хотя бы один из шахов — мат в один, **drop** (уходят в `find-mate-in-one-square`) | `squares[]` (нужно отметить все, score = TP/(TP+FP+FN)) | важна |
...
| 8 | `find-undefended-attack` | Найти ход, который **нападает** на фигуру, не имеющую защитников (создание угрозы, не съедание) | Для каждого хода `m`: после `m` существует `enemy_piece` (кроме короля) под боем, у которой `attackers(_, enemyColor)` = 0. **Канонического выбора среди многих нет** — позиция с >1 такими ходами **drop** (см. §2.2.1 KS-2313). На практике slider-фигуры почти всегда multivalent → composed-drill крайне редкий | `move` (from + to). Promotion в v1 отбрасывается | важна |
