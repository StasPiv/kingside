...
Одновременно закрывается долг v1: полки и шаблоны из `tools/study-plan/` (`rating-shelves.json`, `texts.*.json`) **подключаются к генератору как источник** (границы полок, доли теории/практики, `priorityThemes`), захардкоженные константы и расхождение границ удаляются.
...
|---|---|---|---|---|
| 1 | Ядро v2: урок в занятии | backend | Миграция (`StudySession.lessonId`, `StudyTask.role`), скрытый персональный курс, `StudyLessonBuilder`, интерфейс `LessonMaterialSource` + `BaselineMaterialSource`, генератор v2, подключение `tools/study-plan/*.json` вместо хардкода, ротация уроков, score в сессию | — |
...
| 4 | Тексты и шаблоны уроков | content | Шаблоны вводных text-шагов и заданий самопроверки game-шага по полкам (en/ru) в `tools/study-plan/` — в формате, который читает StudyLessonBuilder (согласовать схему с задачей 1); тексты уведомлений «урок готов» | схема из 1 |
