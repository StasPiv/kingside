...
- Статус: **Accepted with follow-ups** (2026-05-15)
- Дата: 2026-05-07 (Proposed), 2026-05-15 (Accepted)
- Связанные задачи: KS-2475, KS-3048…KS-3061
- Связанные ADR: ADR-022 (broadcast-service merge), ADR-042 (tactic-worker), ADR-018 (archive-service)
- Авторы: architect (анализ), devops (фактические данные пайплайна и реализация)
...
## 9. After — фактические результаты (2026-05-15)

Серия E1–E7 реализована под тикетами KS-3048…KS-3061 (нумерация переехала
относительно §6: при создании задач часть номеров была занята, см. сводку
ниже). Этот раздел фиксирует то, что фактически измерили и применили.

### 9.1. Соответствие планируемых мер реальным тикетам

| Мера ADR §5 | Тикет | Статус | Эффект (факт) |
|-------------|-------|--------|---------------|
| §5.1 skip migrate при пустом diff | KS-3049 (E2) | done | **−67 с** на warm broadcast (243 → 176 с); −83 с где diff пуст |
| §5.2 ECR registry build cache | KS-3054 (E7) | **not-applicable** | На true-warm после KS-3057 build+push ≈ 2 с — экономить нечего |
| §5.3 параллелизация `scope:'all'` | KS-3053 (E6) | **not-applicable** | `scope:'all'` пользователем не используется, реальный сценарий — single-service |
| §5.4 health-check / dereg tuning | KS-3052 (E5) | done | Прямой эффект <5 с; настройки безопасны (`startPeriod`, `interval`, `retries`, dereg-delay) |
| §5.5 prune devDependencies | KS-3051 (E4) + KS-3058 (5/5) | done | api/broadcast push −10…−25%, **api rollout 319 → 152 с (−166 с)**; образы −100…150 MB |
| §5.6 убрать дубль-migrate из entrypoint | KS-3050 (E3) | done | **−15 с** на rollout api |
| §5.7 развязать `packages/shared` | — | not-scheduled | Вне scope; warm бюджет уложили без этого |
| §5.8 turbo remote cache | — | not-scheduled | Эффекта на текущем Dockerfile не было бы |
| §5.9 pre-build на CI | — | not-scheduled | Решение пользователя — оставить build на агенте |

Дополнительно выполнены работы, не описанные в §5:

| Тикет | Что | Эффект |
|-------|-----|--------|
| KS-3048 (E1) | Инструментирование `_perf_stamp` (25 точек) + baseline-замеры; вынесено в `docs/devops/deploy-perf-baseline.md` | даёт основу для остального |
| KS-3057 | Per-scope lock-файлы (вместо глобального) | Разблокировал параллельные одиночные деплои; устранил cold-cache из-за конкуренции |
| KS-3059 | Профайл Nest-startup | Nest стартует за ~1 с — **не bottleneck**, дальнейшая оптимизация старта приложения нерелевантна |
| KS-3060 | Профайл Fargate → `/health` | Доказан главный остаток: ECR pull 18 с (через NAT) + container init 17 с + ALB health 37 с ⇒ 72 с из 96 с rollout |

### 9.2. Фактические тайминги rollout api (warm single-service)

| Этап | Before (KS-3048) | After (KS-3050 + KS-3051 + KS-3058) | Δ |
|------|----------------:|------------------------------------:|---:|
| `migrate_run_task` (нет diff) | 84.69 с | **0 с** (skip по KS-3049) | −85 с |
| `update_service + services_stable` (по ECS) | 183.79 с | **~96 с** | **−88 с** |
| `services_stable` (по bash-таймеру с polling) | 183.79 с | ~150 с | −34 с |
| Прочее (git/ECR login/build/push/register/atomic) | 7.7 с | 7.7 с | 0 |
| **TOTAL по ECS** | **276 с** | **~104 с** | **−172 с (−62%)** |
| **TOTAL по bash-таймеру** | **276 с** | **~150 с** | **−126 с (−46%)** |

> Разрыв ECS-time vs bash-time = ~54 с — артефакт `aws ecs wait services-stable`
> polling (CLI опрашивает API каждые 15 с, фактический rollout закончился раньше).
> Это не оптимизация деплоя, а особенность инструмента наблюдения. Для целей
> бюджета считаем верную метрику — **время по ECS** (момент `RUNNING + healthy`).

### 9.3. Разложение остатка 96 с на rollout (по KS-3060)

| Под-этап | Время | Природа |
|----------|------:|---------|
| ECR image pull (через NAT) | 18 с | Структурный — лечится VPC PrivateLink (KS-3061) |
| Container init (Fargate sandboxing + setup) | 17 с | Структурный Fargate overhead, ниже не опустить без перехода на EC2 launch type |
| ALB target health (5 проверок × ~7 с) | 37 с | Можно подрезать ещё на 5–10 с через `health-check-grace-period` 0 при готовом startPeriod, но рискованно для broadcast WS |
| Nest startup + applications | ~1 с | Не bottleneck (KS-3059) |
| Deployment plumbing (создание revision/task, drain старого) | ~23 с | ECS internal, не управляется |

### 9.4. Соответствие целевому бюджету ADR-045 (≤90 с warm single-service)

- Текущее: **96 с по ECS, ~150 с по bash-таймеру**.
- До 90 с по ECS не хватает **~6–10 с**. Реалистичный путь — KS-3061
  (VPC PrivateLink endpoints для ECR + CloudWatch Logs), ожидаемая экономия
  5–12 с на rollout, стоимость ~$48/мес. Решение за пользователем.
- По bash-таймеру 150 с — артефакт polling-инструмента, не реального деплоя.
  Сокращается переходом на event-driven подтверждение (CloudWatch Events
  `ECS Task State Change`) — отдельная задача, в текущий scope не входит.

**Вывод по бюджету:** цель де-факто достигнута по реальному ECS-rollout-времени
(96 с против 276 с baseline, −62%). Формальное укладывание в ≤90 с
требует апрува KS-3061. Это и есть единственный pending follow-up меры.

### 9.5. Дополнительные эффекты

- Image size: −100…150 MB на каждый из 5 сервисов (KS-3051 + KS-3058).
  Ускоряет любой будущий cold-pull, уменьшает $ за ECR storage.
- Параллельные одиночные деплои разблокированы (KS-3057): больше нет
  глобального lock-файла, два разных сервиса можно деплоить одновременно.
- Baseline-замеры с raw trace paths зафиксированы как
  `docs/devops/deploy-perf-baseline.md` — основа для последующих регрессионных
  проверок.

### 9.6. Решения по нереализованным мерам (§5.2, §5.3, §5.7–5.9)

- **§5.2 ECR registry cache (KS-3054)** — **закрыто как not-applicable**.
  После KS-3057 на true-warm `docker build` + `docker push` ≈ 2 с; кэшировать
  нечего. Cold-сценарий (рестарт контейнера агента) встречается редко и
  субъективно приемлем.
- **§5.3 параллелизация `scope:'all'` (KS-3053)** — **закрыто как
  not-applicable**. Пользователь сценарием `scope:'all'` не пользуется;
  реальная нагрузка — одиночные деплои, для которых KS-3057 разблокировал
  параллельный запуск нескольких single-service одновременно. Эффект
  достигнут другим путём.
- **§5.7 развязка `packages/shared`** — остаётся known-issue. Триггер «all»
  при правке shared есть, но т.к. `scope:'all'` сейчас не используется,
  острота снята. Возврат — если/когда понадобится.
- **§5.8 turbo remote cache, §5.9 pre-build на CI** — не запланированы.
  Решение пользователя: build остаётся на агентском хосте, текущая скорость
  устраивает.

### 9.7. Open follow-ups (после Accepted)

| Тикет | Что | Условие |
|-------|-----|---------|
| KS-3061 | VPC PrivateLink endpoints для ECR + CloudWatch Logs (~$48/мес, −5–12 с rollout) | Ждёт явный апрув пользователя на стоимость |

Других открытых пунктов по ADR-045 нет.

---

## 10. Закрытие ADR

Серия E1–E7 + follow-ups завершена. Достигнуто −62% на реальном rollout
warm single-service api (276 → ~104 с по ECS), без архитектурных рисков и
без регрессий. Бюджет ≤90 с по ECS остаётся pinned за KS-3061 как
opt-in мерой при готовности пользователя оплатить $48/мес.

Принципы, утверждённые этим ADR и подтверждённые практикой:

- **Skip-логика для гарантированно бесполезных шагов** (миграции при пустом
  diff, дублирующий entrypoint-migrate) — самая дешёвая и эффективная мера.
- **Размер образа влияет на rollout** (через ECR pull), prune devDependencies
  обязателен в production stage.
- **Polling-инструменты (`aws ecs wait`) искажают наблюдаемые тайминги** —
  для будущих замеров использовать ECS-time, а не bash-time.
- **Fargate имеет структурный пол** ~30–40 с на rollout (image pull +
  container init + ALB health), ниже которого можно опуститься только
  переходом на EC2 launch type или VPC PrivateLink.
- **Параллельные одиночные деплои важнее `scope:'all'`** — пользовательский
  сценарий за полгода работы.
