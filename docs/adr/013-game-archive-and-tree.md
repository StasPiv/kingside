...
- импорт TWIC — это download zip 5-20MB + распаковка + парсинг тысяч PGN + сотни тысяч UPSERT. Долгая операция (30-90 сек). В процессе API она забьёт event loop и заблокирует остальные REST-запросы.
