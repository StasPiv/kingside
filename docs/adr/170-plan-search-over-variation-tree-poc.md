...
**Дата:** 2026-07-26 (рев. 8 — реализованный и одобренный вариант: поиска в глубину нет; оракул = один прогон ONNX `t1-256x10.onnx` через сервис `leela-oracle.py` (НЕ бинарь lc0), Maia — один прогон, на листе только статический форк trace; приоритет ширины дерева. См. рев. 7 — ограничение роста оракула, рев. 6 — оракул Leela)
**Статус:** Принято (PoC собран на pos2: 40 линий, фаза C работает)
...
**Движки (рев.8, реализованный вариант — одобрен пользователем).** Ходы внутри дерева выбирают две сети, каждая — ОДИН прямой прогон (голова политики + оценочная), без поиска в глубину:
- **Maia-1500** (`@kingside/maia-core`) — человеческая policy: вероятные ходы и их вероятности одним прогоном.
- **Сильная сеть Leela T1 256×10 — «оракул»** объективно сильнейшего хода. Это **один прогон ONNX** `/project/.agent-tmp/t1-256x10.onnx` (голова политики + WDL), ход = argmax политики. **НЕ бинарь lc0:** lc0 с `nodes=1`/VerboseMoveStats скатывался на trivial-backend и давал плоскую/ошибочную политику — от бинаря отказались. Реализация — тёплый python-сервис `tools/adr170-poc/leela-oracle.py` (протокол «FEN → JSON»): энкодер 112 плоскостей `lc0poc.encode` (KS-4989), декод политики 1858 по карте `kAttnPolicyMap` из исходников lc0, при ходе чёрных доска зеркалится на сторону хода. Проверено: после Rc4 сеть даёт Qxc4 = 90.9% (сходится с Maia-2400 = 88%).

**Поиска в глубину нет нигде — приоритет ширины над глубиной.** На листе Stockfish `go depth N` УБРАН; остаётся только статический форк trace (`eval json`, NNUE выключен) — он без поиска и даёт разбиение на 63 составляющие И суммарную статическую оценку (`total.v`). Причина: для PoC важнее **ширина дерева** (охват разных планов), а не глубина движкового анализа каждой линии. Прежние `LEELA_NODES = 1600`, `SF_DEPTH_LEAF = 16` и запуск lc0-бинаря отменены.

**Факт-ограничение статики.** На тактических листьях статическая оценка расходится с поисковой. Пример: лист `Rc4 Qxc4 dxc4` статикой −3.27, поиском +6 (Rd7 ловит ферзя). В дереве это смягчается тем, что линия ведётся глубже развязки (Rxd7), и лист замеряется уже после тактики (+5.4…+6.3). Полный отказ от поиска этот риск не убирает — учитывать при чтении оценок листьев.
...
2. Конечная позиция линии: статический форк trace — суммарная оценка (`total.v`) + разбиение на 63 составляющие; WDL — одним прогоном Leela (см. 5.1 шаг 5). Stockfish `go depth` не запускается.
