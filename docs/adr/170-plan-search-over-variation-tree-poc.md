...
1. Строим варианты (линии) Stockfish/Maia от исходной позиции.
2. Каждой линии даём **одно словесное описание линии**, храним рядом **конечную оценку** линии и её **разбиение на составляющие** (Stockfish + trace).
...
**Что кодируется вектором:** только **одно описание линии**. Не несколько стилей, не ярлыки о замысле, не факторный пересказ — просто описание последовательности ходов линии (обе стороны). Оценка и её разбиение — отдельные хранимые поля записи, НЕ часть эмбеддинга.
...
| `description` | **Одно** описание линии: последовательность ходов обеих сторон, констатация без оценочных ярлыков и без план-обобщений |
...
flowchart TD
  ...
  D --> F[Одно описание линии - ходы обеих сторон]
...
4. Каждой линии — **одно** описание: пересказ ходов обеих сторон (детерминированно из ходов, без LLM и без ярлыков о замысле).
...
Описания генерируются детерминированно из ходов — LLM для их создания не нужен. LLM участвует только в фазе C (формулировка ответа по отобранным линиям).
...
- **backend:** конвейер наполнения (линии Maia+Stockfish, конечная оценка + trace-разбиение, ОДНО описание линии, эмбеддинг Voyage, хранилище) + фаза C (эмбеддинг запроса, векторный поиск топ-5, ответ LLM). Trace — `tools/stockfish-trace/`; Maia — `apps/api/src/position-comment/maia.service.ts`. **Правка к текущей реализации:** убрать три стиля описания и факторный пересказ из текста; описание — одно, только ходы линии; вектор — по этому единственному описанию.
