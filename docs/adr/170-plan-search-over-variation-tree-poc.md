...
**Дата:** 2026-07-25 (рев. 6 — §5.1: ходы дерева ведёт только Lc0 — Maia policy + оракул сильной сети Leela T1 на каждом узле; Stockfish только на листе ради trace-разбиения; отсечка защищает ветки оракула. См. рев. 5 — ветвление по энтропии, рев. 2 — приведение к алгоритму пользователя)
**Статус:** Предложено
...
2. **Кандидатные первые ходы (= плоскость планов).** Объединяем:
   ...
   - top-3 хода сильной сети Leela (multipv) в корне.
...
4. **Остановка линии** — по первому из: достигнута `D_max`; мат/пат; исход стабилизировался (одна из W/D/L > 90% по WDL сильной сети Leela) — «позиция понятна», линия закрывается раньше. WDL на узле берётся у Lc0 (тёплый процесс), Stockfish для этого не запускается.
5. **Замер — только на КОНЕЧНОЙ позиции линии (лист):** Stockfish — оценка + WDL; форк trace (`eval json`, NNUE выключен) — разбиение на составляющие. Внутри линии Stockfish НЕ запускается вовсе — ходы ведут два тёплых процесса Lc0 (Maia + сильная сеть). Это резко сокращает вызовы Stockfish (узкое место — его перезапуск на узел; при замере только листьев их на порядок меньше).
6. **Ограничение объёма:** не более 40 линий. Отсечка при перевыполнении — по вероятности пути Maia (произведение вероятностей ходов линии), оставляя топ-40, **НО** линии, содержащие ход-оракул Leela (флаг из п.3), от отсечки по вероятности Maia защищены — иначе объективно сильные ходы, ради которых оракул и добавлен, выпадут первыми (у них низкая вероятность Maia). Порог P_fork = 0.30 — стартовый; если человеческих развилок окажется устойчиво <20 или >40, настраивается на PoC.
