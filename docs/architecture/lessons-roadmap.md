...
- **Stockfish в браузере с `UCI Skill Level`** ведёт себя неустойчиво на
  старых wasm-сборках. Митигация: тестировать wasm-сборки перед
  релизом; при проблемах с конкретной версией — обновлять wasm-сборку,
  не переключаться на сервер. В разделе «Уроки» используется только
  Stockfish WASM на клиенте (ADR-025 §2.8).
