...
- ~~Логические конфликты (два агента меняют API-контракт несовместимо) не предотвращаются ни одним техническим средством~~ → Решено в [KS-202: Предотвращение логических конфликтов](./logical-conflict-prevention.md)
- При сбое агента (crash, timeout) задача может зависнуть в статусе "В работе" — нужен механизм обнаружения зависших задач
