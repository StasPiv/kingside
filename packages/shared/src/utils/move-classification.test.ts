...
describe('classifyMove — KS-3020 / ADR-066 §5 (WDL primary)', () => {
  ...
  describe('isBestMove override', () => {
    it('isBestMove=true → best независимо от WDL-loss', () => {
      expect(
        classifyMove({
          ...wdlPair(0.50), // catastrophe loss
          isBestMove: true,
        }),
      ).toBe('best');
    });

    it('isBestMove=true перебивает даже mate-edge wdl_after.l', () => {
      // Не должно случаться на проде (нельзя одновременно сходить в мат и быть PV1),
      // но defensive — override срабатывает первым.
      expect(
        classifyMove({
          wdlBefore: { w: 1000, d: 0, l: 0 },
    ...
    it('isBestMove=false → нормальная классификация', () => {
      expect(
        classifyMove({
          ...wdlPair(0.08),
          isBestMove: false,
