...
describe('screenGameForCandidates (KS-3386 / ADR-083)', () => {
  // deltaWThreshold=0.6, screenDeltaMargin=0.15 → screenThreshold=0.45.
  const SCREEN_SETTINGS: PuzzleGenSettings = {
    ...DEFAULT_SETTINGS,
    screenEnabled: true,
  ...
  function step(overrides: Partial<PlyStep>): PlyStep {
    return {
      ply: 20,
      fenBefore: 'fenB',
      fenAfter: 'fenA',
  ...
  /** Мок-движок: отдаёт WDL POV side-to-move по FEN-ключу. */
  function makeEngine(wdlByFen: Record<string, { w: number; d: number; l: number }>) {
    const calls: string[] = [];
    const engine: PuzzleGenEngine = {
      analyze: async (fen) => {
        calls.push(fen);
        const wdl = wdlByFen[fen];
        if (!wdl) return [];
        return [pv('x1x1', wdl)];
      },
    };
  ...
  it('POV-нормализация: white-to-move зевнул → approxDeltaW корректен → кандидат', async () => {
    // fenB POV white (зевнул белый): w=900. fenA POV black (соперник):
    // raw {w:850,d:100,l:50} → POV white invert {w:50,d:100,l:850} →
    // w зевнувшего(white) после = 50. approxDeltaW = (900−50)/1000 = 0.85.
    const { engine } = makeEngine({
      fenB: { w: 900, d: 50, l: 50 }, // POV white (side=w)
      fenA: { w: 850, d: 100, l: 50 }, // POV black (side=b)
    });
    const r = await screenGameForCandidates(
      [step({ ply: 20, preventiveSolverSide: 'w', reactiveSolverSide: 'b' })],
      engine,
      SCREEN_SETTINGS,
    );
    expect(r.candidatePlys.has(20)).toBe(true);
  });

  it('POV-нормализация: black-to-move зевнул → инверсия учтена → кандидат', async () => {
    // fenB POV black (зевнул чёрный): raw {w:900,d:50,l:50}. POV white
    // invert {w:50,d:50,l:900}. w зевнувшего(black) до = white.l = 900.
    // fenA POV white (соперник): raw {w:850,d:100,l:50} (as-is POV white).
    // w зевнувшего(black) после = white.l = 50. approxDeltaW = 0.85.
    const { engine } = makeEngine({
      fenB: { w: 900, d: 50, l: 50 }, // POV black (side=b)
      fenA: { w: 850, d: 100, l: 50 }, // POV white (side=w)
    });
    const r = await screenGameForCandidates(
      [step({ ply: 21, preventiveSolverSide: 'b', reactiveSolverSide: 'w' })],
      engine,
      SCREEN_SETTINGS,
    );
    expect(r.candidatePlys.has(21)).toBe(true);
  });

  it('отсев по margin: approxDeltaW ниже порога → НЕ кандидат', async () => {
    // fenB white w=600, fenA POV black raw {w:0,d:50,l:950}→white{w:950..}
    // w зевнувшего(white) после = invert(raw).w = raw.l = 950.
    // approxDeltaW = (600−950)/1000 = −0.35 < 0.45 → не кандидат.
    const { engine } = makeEngine({
      fenB: { w: 600, d: 200, l: 200 },
      fenA: { w: 0, d: 50, l: 950 },
    });
    const r = await screenGameForCandidates(
      [step({ ply: 22, preventiveSolverSide: 'w', reactiveSolverSide: 'b' })],
      engine,
      SCREEN_SETTINGS,
    );
    expect(r.candidatePlys.has(22)).toBe(false);
  });

  it('граница порога: approxDeltaW == screenThreshold (0.45) → кандидат (≥)', async () => {
    // fenB white w=950; fenA POV black raw {w:?}, нужно w зевнувшего после
    // = 500 → raw.l = 500. approxDeltaW = (950−500)/1000 = 0.45 == порог.
    const { engine } = makeEngine({
      fenB: { w: 950, d: 30, l: 20 },
      fenA: { w: 0, d: 500, l: 500 }, // POV black: l=500 → white.w=500
    });
    const r = await screenGameForCandidates(
      [step({ ply: 23, preventiveSolverSide: 'w', reactiveSolverSide: 'b' })],
      engine,
      SCREEN_SETTINGS,
    );
    expect(r.candidatePlys.has(23)).toBe(true);
  });

  it('single-pass дедуп: 3 последовательных ply → 4 уникальные позиции, 4 вызова движка', async () => {
    // fenAfter[i] == fenBefore[i+1]: p0..p3 разделяют позиции.
    const steps: PlyStep[] = [
      step({ ply: 20, fenBefore: 'p0', fenAfter: 'p1', preventiveSolverSide: 'w', reactiveSolverSide: 'b' }),
      step({ ply: 21, fenBefore: 'p1', fenAfter: 'p2', preventiveSolverSide: 'b', reactiveSolverSide: 'w' }),
      step({ ply: 22, fenBefore: 'p2', fenAfter: 'p3', preventiveSolverSide: 'w', reactiveSolverSide: 'b' }),
    ];
    const { engine, calls } = makeEngine({
      p0: { w: 500, d: 300, l: 200 },
      p1: { w: 500, d: 300, l: 200 },
      p2: { w: 500, d: 300, l: 200 },
      p3: { w: 500, d: 300, l: 200 },
    });
    const r = await screenGameForCandidates(steps, engine, SCREEN_SETTINGS);
    // 4 уникальные позиции (p0,p1,p2,p3), не 6 (2×3).
    expect(calls.length).toBe(4);
    expect(r.evalCurve.size).toBe(4);
  });

  it('терминальная позиция (isGameOverAfter) → кандидат, fenAfter не анализируется', async () => {
    const { engine, calls } = makeEngine({
      fenB: { w: 500, d: 300, l: 200 },
      // fenA отсутствует — не должно запрашиваться.
    });
    const r = await screenGameForCandidates(
      [step({ ply: 24, isGameOverAfter: true })],
      engine,
      SCREEN_SETTINGS,
    );
    expect(r.candidatePlys.has(24)).toBe(true);
    // fenAfter ('fenA') не запрашивался; fenBefore тоже пропущен (continue
    // до анализа) — движок не вызван вовсе для терминального ply.
    expect(calls).not.toContain('fenA');
  });

  it('engine не вернул WDL (пустой массив) → консервативно кандидат', async () => {
    const { engine } = makeEngine({
      fenB: { w: 900, d: 50, l: 50 },
      // fenA нет в карте → analyze вернёт [] → eval null → кандидат.
    });
    const r = await screenGameForCandidates(
      [step({ ply: 25, preventiveSolverSide: 'w', reactiveSolverSide: 'b' })],
      engine,
      SCREEN_SETTINGS,
    );
    expect(r.candidatePlys.has(25)).toBe(true);
  });

  it('screenEnabled=false → все ply кандидаты, движок не вызван, evalCurve пуст', async () => {
    const { engine, calls } = makeEngine({});
    const steps = [
      step({ ply: 20 }),
      step({ ply: 21 }),
    ...
    const r = await screenGameForCandidates(steps, engine, {
      ...SCREEN_SETTINGS,
      screenEnabled: false,
    });
    expect(r.candidatePlys).toEqual(new Set([20, 21, 22]));
    expect(calls.length).toBe(0);
    expect(r.evalCurve.size).toBe(0);
  });

  it('screen* поля не заданы (undefined) → screenEnabled falsy → все ply кандидаты', async () => {
    const { engine, calls } = makeEngine({});
    // DEFAULT_SETTINGS без screen-полей (одно­фазный caller).
    const r = await screenGameForCandidates(
      [step({ ply: 30 }), step({ ply: 31 })],
      engine,
      DEFAULT_SETTINGS,
    );
    expect(r.candidatePlys).toEqual(new Set([30, 31]));
    expect(calls.length).toBe(0);
  });
});
