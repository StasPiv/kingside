...
export type PuzzleDto = {
   ...
   * `{firstMovePV1} ∪ MaiaTopK`). Заполняется offline-разметкой через
   ...
   * проходит фильтр, если значение `>=` порога
   * `PRECISION_MAIA_DEFAULT_THRESHOLD` (default 0.3, см. F1).
   *
   ...
   * `maiaWeakChoiceProb` (`1` = ADR-106 §2.1). `null` — значение либо
   * не размечено, либо унаследовано от отменённой ADR-104 и потому
   * семантически непригодно для нового фильтра. Фронт активирует
   * фильтр только для строк с актуальной версией (см. ADR-106 §5).
   */
  maiaMetricVersion?: number | null;
  /**
   * KS-3631 / ADR-104 §4 (поле сохранено в ADR-106 §2.5). ELO, под
   * которым прогоняли Maia-разметку (для воспроизводимости и проверки
