...
export const PUZZLE_GEN_DEFAULTS = {
  ...
  skipDecidedWdl: 0.95,
  /**
   * KS-3135 / ADR-068 §3.2, §5.4. Для триггера по `deltaW`: минимальная
   * вероятность победы решающего сразу после хода блaндера
   * (`W_after_for_solver = wdl_after_raw.w / 1000`). Защита от пазлов,
   * где «шанс на победу» — мираж.
   */
  minWAfterForSolver: 0.5,
  /**
   * KS-3135 / ADR-068 §3.2, §5.4. Для триггера по `deltaD`: минимальная
   * сумма W+D решающего сразу после хода блaндера (хотя бы «держать
   * ничью»). Защита от пазлов, где решающий после зевка всё ещё в
   * проигрышной позиции.
   */
