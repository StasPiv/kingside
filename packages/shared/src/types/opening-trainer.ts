...
export interface RepertoireTree {
  ...
  meta: {
    ...
    maxDepth: number;
    /**
     * KS-4105. Нефатальные предупреждения парсинга PGN — например,
     * пропущенные null-move ветки (`Z0` / `--` из ChessBase: пропуск
     * хода, не тренируемый). Импорт такие ветки не обрывают, а
     * пропускают и фиксируют здесь. Отсутствует/пусто — предупреждений
     * нет (поле опционально для обратной совместимости).
     */
    warnings?: string[];
  };
}
