 ...
 * для Maia-аннотации.
 *
 * Дубль `apps/tactic-worker/src/maia/solution-uci.ts` — в prod-Docker
 * production-stage копирует только `apps/tactic-worker/dist`, исходники
 * не доступны для импорта из `tools/`. Не вводим зависимость между
 * tools/ и apps/.
 *
