...
const TOOLS = [
  ...
  { name: 'deploy', description: 'Запустить деплой одного сервиса на AWS — атомарно, один scope за вызов. scope (обязателен): frontend | api | game-service | broadcast-service | archive-service | tactic-worker | synthetic-bot. ЗАПРЕЩЕНО: "all", "workers", "" (auto) — webhook вернёт 400. Если нужно несколько сервисов — несколько последовательных вызовов с явным scope.',
    inputSchema: { type: 'object', properties: { scope: { type: 'string', enum: ['frontend', 'api', 'game-service', 'broadcast-service', 'archive-service', 'tactic-worker', 'synthetic-bot'] } }, required: ['scope'] } },
