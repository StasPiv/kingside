...
def add_comment(key: str, data: CommentCreate):
    conn = get_db()
    row = conn.execute("SELECT status, assignee FROM issues WHERE key = ?", (key,)).fetchone()
    if not row:
        raise HTTPException(404, f"Issue {key} not found")
    # В in_progress задаче может писать только её assignee.
    # Координатор должен сначала перевести в To Do (transitionId: 11).
    if row["status"] == "in_progress" and data.author != row["assignee"]:
        raise HTTPException(
            400,
            f"Issue {key} is 'in_progress' and assigned to '{row['assignee']}'. "
            f"Only the assignee can comment. Transition to 'To Do' first (transitionId: 11).",
        )
    ts = now_iso()
    with db() as c:
        c.execute(
