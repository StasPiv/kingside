...
- 🔴 АБСОЛЮТНЫЙ ЗАПРЕТ: если что-то в окружении не работает (dev-сервер не стартует, API недоступен, CORS ошибки, auth не проходит, модули не резолвятся) — ты ОБЯЗАН немедленно прекратить работу. Никаких workaround, никаких mock, никаких попыток обойти проблему. Добавь комментарий `LAYOUT: Окружение не готово: <проблема>. @coordinator` и ЗАВЕРШИ РАБОТУ. Это не рекомендация — это запрет на продолжение. Любая попытка обойти проблему окружения = нарушение инструкции.
- После завершения работы:
  ...
  4. Добавь комментарий с результатом
  5. Переведи задачу в статус Done (transitionId: 41)
