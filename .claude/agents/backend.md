...
- **Если окружение не работает** (не запускаются тесты, не находятся бинари, не резолвятся модули и т.д.) — НЕ пытайся чинить окружение самостоятельно. ОСТАНОВИСЬ и добавь комментарий: `BACKEND: Окружение не готово: <описание проблемы>. @coordinator` — координатор создаст задачу на devops
