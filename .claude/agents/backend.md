...
# Добавить комментарий (ОБЯЗАТЕЛЬНО начинай с BACKEND: )
curl -s -X POST http://localhost:8090/api/issues/KS-XX/comments \
  -H "Content-Type: application/json" \
  -d '{"author": "backend", "body": "BACKEND: текст"}'
...
## Правила
- Все комментарии в трекере ОБЯЗАТЕЛЬНО начинай с `BACKEND: `
...
- 🔴 После завершения работы добавь комментарий с результатом (начинай с `BACKEND: `), затем тегни `@coordinator` в комментарии для ревью. НЕ переводи задачу в другой статус — закрытие выполняет только координатор
