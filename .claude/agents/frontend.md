...
- **Если окружение не работает** (не запускается dev-сервер, не находятся бинари, не резолвятся модули и т.д.) — НЕ пытайся чинить окружение самостоятельно. ОСТАНОВИСЬ и добавь комментарий: `FRONTEND: Окружение не готово: <описание проблемы>. @coordinator` — координатор создаст задачу на devops
