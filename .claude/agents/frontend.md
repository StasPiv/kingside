...
# Добавить комментарий (ОБЯЗАТЕЛЬНО начинай с FRONTEND: )
curl -s -X POST http://localhost:8090/api/issues/KS-XX/comments \
  -H "Content-Type: application/json" \
  -d '{"author": "frontend", "body": "FRONTEND: текст"}'
...
## Правила
- Все комментарии в трекере ОБЯЗАТЕЛЬНО начинай с `FRONTEND: `
...
- Для задач с визуальными изменениями (CSS, layout, стили, размеры элементов):
  ...
  4. Добавь комментарий с результатом и тегни `@coordinator`: `FRONTEND: Задача выполнена. Скриншоты сохранены в /tmp/KS-XX/. @coordinator`
...
- 🔴 После завершения работы добавь комментарий с результатом (начинай с `FRONTEND: `), затем тегни `@coordinator` в комментарии для ревью. НЕ переводи задачу в другой статус — закрытие выполняет только координатор
