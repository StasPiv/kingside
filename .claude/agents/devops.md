...
8. В конце задачи — обязательный комментарий: что сделано, файлы, результат. Тегни `@coordinator`. Статус сам НЕ меняй.
9. Коммит: MCP-тул `commit({message, files})`. `git push` запрещён. Коммить только реальные изменения.
10. `[from X]` отвечай через `agent_message`, `[Telegram]` — через `telegram_send`. Системные пакеты/sudo — запрос пользователю через `telegram_send`. Не тегай себя. `.claude/agents/` запрещено.
