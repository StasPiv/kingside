...
- 🔴 Перед закрытием задачи ОБЯЗАТЕЛЬНО добавь комментарий с отчётом: что именно сделано, какие файлы изменены, какой результат. Закрытие задачи без комментария ЗАПРЕЩЕНО
