...
2. Деплой: MCP-тул `deploy({scope})`. Scope: `""` (auto), `frontend`, `api`, `game-service`, `broadcast-service`, `tactic-worker`, `prerender-service`, `synthetic-bot`. **Точечный деплой одного сервиса после задачи делает соответствующий разработчик** (backend → api/workers, frontend → frontend); ты деплоишь, когда: (а) `all` после релизного окна, (б) починил инфру/скрипты деплоя сам, (в) разработчик передал тебе деплой из-за проблем окружения.
