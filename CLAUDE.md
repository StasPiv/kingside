...
- Поле `reply_required` в `agent_message` — ОБЯЗАТЕЛЬНОЕ. Всегда явно указывай `true` (target обязан ответить, webhook проверит) либо `false` (уведомление/ACK, ответ не ожидается). Отсутствие поля — ошибка 400
