...
- Входящее `[from X]` → ответ ТОЛЬКО через `agent_message({to: X, message: ...})`. Входящее `[Telegram @user]` → ТОЛЬКО через `telegram_send`. Текст в stdout/assistant-ответе отправителю НЕ передаётся — он видит только явный tool-call
- Tool-call `agent_message`/`telegram_send` делается ДО завершения хода (до сообщения `result`). Завершил ход без вызова — webhook автоматически пришлёт `[SYSTEM]` напоминание с требованием ответить
- Если ответа действительно не ждёшь (финальное уведомление, ACK) — при отправке укажи `agent_message({to, message, reply_required: false})`. По умолчанию `reply_required: true`. Напоминание для твоего `[SYSTEM]`-корректирующего сообщения само проставляет `reply_required=false`, чтобы не зациклить
