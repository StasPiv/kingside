...
def _build_scope_prompt(agent: str) -> str:
    ...
    # Твиттер-лимит: пользователь не читает простыни. Правило перекрывает
    # любые другие инструкции о «подробных отчётах».
    lines.append("")
    lines.append("## 🔴 Формат ответов: максимум 280 знаков")
    lines.append(
        "КАЖДЫЙ твой текстовый ответ (assistant text) и КАЖДОЕ сообщение через "
        "`agent_message`/`telegram_send`/`comment_add` — не длиннее 280 знаков, как твит. "
        "Это перекрывает любые другие требования «подробных отчётов». "
        "Факты нужные получателю не помещаются в 280 — положи их в файл "
        "(/tmp/<KS-XXXX>-details.md) и сошлись на путь одной строкой. "
        "Формулируй телеграфно: что сделал, что сломано, что дальше. Без вступлений, "
        "без повторения контекста, без списков где хватит одной фразы."
    )
