...
import {
  ...
  type WsMatchmakingNoOpponentsPayload,
  type WsErrorPayload,
...
export class MatchmakingGateway implements OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    // для того, и для другого, чтобы не плодить connection'ы.
    this.subRedis
      .subscribe(MATCHMAKER_FOUND_CHANNEL, MATCHMAKER_NO_OPPONENTS_CHANNEL)
      .catch((e) =>
    ...
    this.subRedis.on('message', (channel: string, message: string) => {
      try {
        if (channel === MATCHMAKER_FOUND_CHANNEL) {
          this.handleFoundMessage(message);
        } else if (channel === MATCHMAKER_NO_OPPONENTS_CHANNEL) {
          this.handleNoOpponentsMessage(message);
        }
      } catch (e: any) {
    ...
    this.logger.log(
      `Subscribed to ${MATCHMAKER_FOUND_CHANNEL} + ${MATCHMAKER_NO_OPPONENTS_CHANNEL} Redis channels`,
    );
  }

  private handleFoundMessage(message: string): void {
  ...
  /**
   * KS-2197. Получаем `{userId, category, timeInitial, increment, waitedMs}`
   * от `MatchmakingService.sweepNoOpponents`. Шлём WS-event пользователю и
   * чистим `PLAYER_QUEUES_KEY` (LEAVE-эквивалент). Сама запись в zset
   * `matchmaking:<category>` уже удалена сервисом до публикации.
   */
  private handleNoOpponentsMessage(message: string): void {
    ...
    const payload: WsMatchmakingNoOpponentsPayload = {
      ...
      tc: { timeInitial: data.timeInitial, increment: data.increment },
      waitedMs: data.waitedMs,
    };

    this.server
      .to(`user:${data.userId}`)
      .emit(MatchmakingEvents.NO_OPPONENTS, payload);

    // Гарантируем, что повторный JOIN не упирается в `ALREADY_IN_QUEUE`
    // — после server-side LEAVE индекс должен быть чист.
    this.redis.hdel(PLAYER_QUEUES_KEY, data.userId).catch(() => {});

    this.logger.log(
      `matchmaker:no_opponents → ${data.userId.slice(0, 8)} ${data.category} waited=${data.waitedMs}ms`,
    );
  }
