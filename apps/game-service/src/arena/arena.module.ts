...
export class ArenaModule implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.gameService.onGameEnd(async (gameId: string) => {
      ...
      // Clear playing flags so players can seek again
      const flagDelStart = Date.now();
      await this.redis.del(`arena:${tournamentId}:playing:${game.whiteId}`, `arena:${tournamentId}:playing:${game.blackId}`).catch(() => {});
      this.logger.log(`onGameEnd SREM key=${apKey} white=${game.whiteId.slice(0, 8)} black=${game.blackId.slice(0, 8)} removed=${sremResult} flagDel=${Date.now() - flagDelStart}ms`);
      ...
      if (!updated || updated.status === 'finished') {
        await this.gateway.emitRoundEnd(tournamentId, t.currentRound, null);
      ...
      await this.gateway.emitRoundEnd(tournamentId, t.currentRound, nextRoundStartsAt);
      await this.gateway.emitStandings(tournamentId);
  ...
  private async startNextRoundWithEmit(tournamentId: string) {
    try {
      ...
      if (round) {
        ...
        await this.gateway.emitRoundStart(tournamentId, t.currentRound, pairingsPayload);
