...
import { filterAlreadyImported, toBytes } from './dedup.js';
...
describe('filterAlreadyImported', () => {
  it('возвращает все партии, если ни одной нет в БД', async () => {
    ...
    const prisma = {
      archiveGame: { findMany: vi.fn().mockResolvedValue([]) },
    };
    ...
    const prisma = {
      archiveGame: {
        findMany: vi.fn().mockResolvedValue([
          // `contentHash` возвращается Prisma как Buffer — имитируем.
  ...
  it('KS-1621 Scenario: повторный прогон того же батча → все отфильтрованы', async () => {
    ...
    // Симулируем второй проход: в БД уже все три.
    const prisma = {
      archiveGame: {
        findMany: vi.fn().mockResolvedValue(
