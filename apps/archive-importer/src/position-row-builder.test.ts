...
describe('buildPositionRowsForGame', () => {
  it('Gherkin: партия 20 полуходов → 21 строка ply 0..20 с ожидаемыми полями', () => {
    // 20 полуходов итальянской партии — корректные валидные ходы без транспозиций.
    const uci = [
      'e2e4', 'e7e5',
      'g1f3', 'b8c6',
      'f1c4', 'f8c5',
  ...
  it('Gherkin: транспозиция → одна строка per-game', () => {
    // Партия приходит в одну и ту же позицию дважды через транспозицию.
    // Классика: 1. e4 Nc6 2. Nc3 Nb8 3. Nb1 Nc6 → после 2.Nc3 и после 3.Nb1
    // позиции одинаковые по side-to-move и фигурам, но у нас position_key
    // считается по доске (без счётчиков), так что эти позиции совпадут.
    // Проще — повторим стартовую позицию: 1. Nf3 Nf6 2. Ng1 Ng8 → стартовая.
    ...
    // Без дедупа было бы 5 строк (ply 0..4). С дедупом — меньше:
    // ply 0 (start), ply 1 (after Nf3), ply 2 (after Nf6), ply 3 (after Ng1),
    // ply 4 (after Ng8) — но ply 4 = стартовая (ply 0) по позиционному ключу.
    const keys = rows.map((r) => r.positionKey.toString('hex'));
    const unique = new Set(keys);
    expect(unique.size).toBe(rows.length); // все ключи разные в результате

    // Стартовая позиция сохранена только один раз — с ply 0.
  ...
  it('Gherkin: партия 60 полуходов → только ply 0..POSITION_PLY_LIMIT', () => {
    ...
    // Генерим первый легальный ход каждую итерацию, пока не сделаем 60 ходов.
    for (let i = 0; i < 60; i++) {
...
describe('serializePositionRowCsv', () => {
  ...
  it('null-поля сериализуются как пустые поля (COPY NULL \'\')', () => {
    ...
    const csv = serializePositionRowCsv(row);
    // Пустые: move_uci, played_at, avg_elo, result
