/**
 * KS-2411 / ADR-042 — SSL-конфиг для подключения индексера к archive-RDS.
 *
 * Контекст: индексер `tactic-drill` (CLI `index-tactic-drills`,
 * EventBridge schedule) подключается к
 ...
 *
 * AWS RDS global CA bundle лежит в `apps/tactic-worker/certs/rds-ca.pem`
 * (вшивается Dockerfile'ом). Nest-CLI assets копируют его в
 * `dist/certs/rds-ca.pem` для production. Также прописан в
 * `NODE_EXTRA_CA_CERTS`, что подстрахует другие TLS-соединения, но
 * pg-pool сам по умолчанию игнорирует системный trust-store и требует
 * явный ca в config.ssl.
 *
 * ENV-переключатели:
...
export function loadRdsCaBundle(): Buffer | null {
  ...
  // production (Docker): dist/lib/pg-ssl.js → ../certs/rds-ca.pem
  // = dist/certs/rds-ca.pem (nest-cli assets копирует certs/rds-ca.pem
  // в dist/certs/ при сборке).
  candidates.push(resolvePath(__dirname, '..', 'certs', 'rds-ca.pem'));
  // dev: src/lib/pg-ssl.ts → ../../certs/rds-ca.pem
  // = apps/tactic-worker/certs/rds-ca.pem.
  candidates.push(resolvePath(__dirname, '..', '..', 'certs', 'rds-ca.pem'));
