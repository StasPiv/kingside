 ...
 *     `stockfishBestUci`. Если ни на одном — статус `above-range`.
 *
...
export function usePositionMaiaRating(
  ...
  const compute = useCallback(
    async (fen: string, stockfishBestUci: string): Promise<void> => {
      ...
      setState({ status: 'computing', rating: null, error: null });
      ...
      try {
        ...
        if (matched != null) {
          setState({ status: 'done', rating: matched, error: null });
        } else {
          setState({ status: 'above-range', rating: null, error: null });
        }
      } catch (err) {
        if (requestId !== requestIdRef.current) return;
        setState({
  ...
  const reset = useCallback(() => {
    ...
    setState({ status: 'idle', rating: null, error: null });
  }, []);
