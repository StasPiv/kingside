 ...
 *
 * # KS-2039: заголовки секций убраны
 *
 * До KS-2039 над каждым блоком на странице курса рендерился
 * `<h2>` с переводом `lessons.block.<key>`. Пользователь решил, что
 * это лишняя информация — каждый title урока сам несёт раздел
 * (§N), заголовок-секция дублировал контекст. Сейчас `CoursePage`
 * рендерит уроки плоским списком: `blocks.flatMap(b => b.lessons)`.
 * Группировщик остаётся — он отвечает за порядок (см. KS-2038).
 */
