...
/**
 * KS-2566: набор шахматных фигур «Cburnett» (стандарт react-chessboard,
 * лицензия CC BY-SA 3.0; тот же набор, что в трансляциях `/broadcasts`).
 *
 * Перенесён из `react-chessboard/dist/index.esm.js` (defaultPieces) в
 * наш модуль как статические JSX-элементы, чтобы:
 *   - не мaунтить `<Chessboard>` ради красивых фигур (KS-2563 perf-fix);
 *   - не тащить лишний tree-shake-нечитаемый импорт на /puzzles.
 *
 * Каждый элемент — содержимое исходного `<svg viewBox="0 0 45 45">`
 * (без обёртки), чтобы PuzzleMiniBoard мог разместить его через
 * `<g transform="translate(x,y) scale(1/45)">`. Цвет фигур фиксирован:
 * белые — `#ffffff` fill / `#000000` stroke; чёрные — наоборот.
 */

export type PieceFenChar =
  | 'P'
  | 'N'
  | 'B'
  | 'R'
...
const wPiece: React.CSSProperties = {
  opacity: 1,
  fillOpacity: 1,
  fillRule: 'evenodd',
  stroke: '#000000',
...
const wRook: ReactElement = (
  <g style={{ ...wPiece, fill: '#ffffff' }}>
...
const wKnight: ReactElement = (
  <g style={{ ...wPiece, fill: 'none' }}>
...
const bRook: ReactElement = (
  <g style={{ ...wPiece, fill: '#000000' }}>
    <path d="M 9,39 L 36,39 L 36,36 L 9,36 L 9,39 z " style={{ strokeLinecap: 'butt' }} />
...
/**
 * Pre-built JSX-элементы фигур. ViewBox исходных SVG — `0 0 45 45`,
 * поэтому при использовании в нашей доске нужно `<g transform="translate(x,y) scale(1/45)">`.
 */
export const PIECE_SVG: Record<PieceFenChar, ReactElement> = {
  P: wPawn,
  R: wRook,
  N: wKnight,
  B: wBishop,
...
export function isPieceChar(ch: string): ch is PieceFenChar {
  return ch in PIECE_SVG;
}
