/**
 * KS-3634 / ADR-104 §8. Подобрать следующий Precision-пазл с учётом
 * клиентского фильтра Maia. Делает до `maxRetries` повторных вызовов
 * `precisionApi.pickNext` + `puzzleApi.getById` если выпадает
 * неподходящий (Maia-prob выше порога). После исчерпания попыток
 * возвращает последний полученный — чтобы не зависать на «нет пазла».
 *
 * Backend `/precision/next` возвращает только `puzzleId`, без полного
 * PuzzleDto. Чтобы посмотреть `maiaTop1Prob`, делаем дополнительный
 * `GET /puzzles/:id`. Это компромисс производительности до миграции
 * фильтра на сервер (если потребуется).
 */
import type {
...
export async function pickEligiblePrecisionPuzzle(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    // неподходящими, вернём этот как fallback (ADR-104 §8: «лучше
    // показать лёгкий, чем ничего»).
