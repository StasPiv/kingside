 ...
 *
 * KS-4098/KS-4102: чистые функции метрики импортируются из
 * браузер-безопасного входа `@kingside/maia-core/browser` — он НЕ тянет
 * node-провайдер (onnxruntime-node / node:fs), поэтому production vite
 * build не падает. Корневой `@kingside/maia-core` во фронте использовать
 * НЕЛЬЗЯ (реэкспорт node-provider ломает браузерный бандл — см. KS-4096).
 * Сама формула — ровно та же, что на сервере (без дублирования
 * математики). Сведение клиентской копии движка `apps/web/src/lib/maia/`
 * к maia-core — отдельная будущая задача.
 */
import {
