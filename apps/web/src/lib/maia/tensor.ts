/**
 * KS-3577. Препроцессинг FEN → входной тензор Maia-3.
 *
 * Адаптировано из CSSLab/maia-platform-frontend (`src/lib/engine/tensor.ts`,
 * лицензия MIT). Изменения:
 *  - заменён `chess.ts` (там зависимость) на наш `chess.js` v1 (API
 *    совместим: `new Chess(fen)`, `board.moves({ verbose: true })`,
 *    `move.from/to/promotion`);
 *  - часть для Maia-2 (`boardToTensor`, `preprocess`, словари
 *    `all_moves.json`) вырезана — мы используем только Maia-3;
 *  - доустал JSDoc на русском.
 *
 * Maia-3 input layout (см. оригинальный воркер CSSLab):
