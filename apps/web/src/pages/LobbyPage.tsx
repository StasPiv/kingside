...
import { api } from '../api';
import { MatchmakingEvents, type CustomTimeControl, type CreateGameResponse, type RatingFilterMode, type RatingFilter } from '@kingside/shared';
...
export function LobbyPage() {
  ...
  const buildRatingFilter = (): RatingFilter | undefined => {
    if (ratingFilterMode === 'none') return undefined;
    if (ratingFilterMode === 'absolute') {
      return { mode: 'absolute', minRating: ratingMin, maxRating: ratingMax };
    }
    return { mode: 'relative', minus: ratingMinus, plus: ratingPlus };
  };
  ...
  const handleSearch = () => {
    if (searching) {
      ...
      matchmakingSocket.emit(MatchmakingEvents.JOIN, {
        ...
        ratingFilter: buildRatingFilter(),
      });
      setSearching(true);
    }
  };
