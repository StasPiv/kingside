...
/**
 * Минимальный Prisma-surface, нужный indexer'у. Позволяет подсунуть
 * lightweight-мок в unit-тестах без полного `PrismaClient`.
 */
export interface PositionIndexerPrisma {
  $executeRaw: (query: Prisma.Sql) => Promise<number>;
  $transaction: <T>(arg: Array<Promise<T>>) => Promise<T[]>;
}
...
export class PositionIndexerService {
   ...
   *
   * `sourceCode` нужен только для метрик
   * (`position_stats_upsert_duration_seconds{source}`) — инжектируется
   * caller'ом, чтобы один singleton PositionIndexerService обслуживал
  ...
  async index(
    ...
    await this.metrics.timePositionStatsUpsert(sourceCode, async () => {
      const deltas = this.collectDeltas(games, bucket);
      if (deltas.size === 0) return;
      await this.applyDeltas(this.prisma as unknown as PositionIndexerPrisma, deltas);
    });
  }

  /**
   * Вариант индекса для тестов/CLI — принимает готовый prisma surface,
   * минуя DI. Используется в `rebuild-position-stats`, где логика не
   * требует ничего кроме query runner'а.
   */
  async indexWithPrisma(
    prisma: PositionIndexerPrisma,
    games: ParsedGame[],
    sourceCode: string,
    bucket: string = DEFAULT_BUCKET,
  ): Promise<void> {
    ...
    await this.metrics.timePositionStatsUpsert(sourceCode, async () => {
      const deltas = this.collectDeltas(games, bucket);
      if (deltas.size === 0) return;
      await this.applyDeltas(prisma, deltas);
    });
  }
