...
import { OpeningTrainerDemoService } from './opening-trainer-demo.service';
import { OpeningTrainerAdminController } from './admin/opening-trainer-admin.controller';
import { OpeningTrainerAdminService } from './admin/opening-trainer-admin.service';
 ...
 *
 * KS-4674 / ADR-146: добавлены `OpeningTrainerAdminController` (CRUD
 * демо-репертуаров под `AdminOrServiceGuard` + scope `repertoire:write`)
 * и `OpeningTrainerDemoService` (чтение демо из БД для публичных
 * `/opening-trainer/demo*`). `DemoRepertoireSeedService` удалён из
 * runtime — переехал в bootstrap-скрипт `scripts/seed-demo-repertoires.ts`.
 */
@Module({
  ...
  providers: [
    ...
    // KS-4674 / ADR-146 §2.3: чтение демо-репертуаров из БД.
    OpeningTrainerDemoService,
    // KS-4674 / ADR-146: админский CRUD демо-репертуаров.
