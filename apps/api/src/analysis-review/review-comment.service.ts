...
export class ReviewCommentService {
  ...
  buildSystemPrompt(language: 'en' | 'ru', _userElo: number): string {
    if (language === 'ru') {
      return [
        'Ты комментируешь шахматные позиции строго на основании поданных позиционных критериев.',
        '',
        'Вход: JSON-массив записей. В каждой записи — FEN позиции и набор позиционных факторов (positional_shifts, positional_subterms, tactical_motifs, hanging_piece, threats_created, threats_missed, sf_best и др.).',
        '',
        'Задача: для каждой записи дать ОДНО короткое предложение на русском, опираясь ТОЛЬКО на поданные критерии. Не выдумывай мотивы, фигуры, угрозы и оценки, которых нет во входе. Если на основе поданных критериев сказать нечего — верни пустую строку "".',
        '',
        'Выход: JSON-массив строк той же длины и в том же порядке, что и вход.',
      ].join('\n');
    }
    return [
      'You comment on chess positions strictly based on the provided positional factors.',
      '',
      'Input: JSON array of records. Each record contains the position FEN and a set of positional factors (positional_shifts, positional_subterms, tactical_motifs, hanging_piece, threats_created, threats_missed, sf_best and others).',
      '',
      'Task: for each record write ONE short English sentence based ONLY on the provided factors. Do not invent motifs, pieces, threats or evaluations that are not in the input. If there is nothing to say based on the provided factors — return an empty string "".',
      '',
