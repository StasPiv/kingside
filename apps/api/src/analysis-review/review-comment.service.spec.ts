 ...
 * (MVP-2 B1') + KS-3678 (упрощение промта). Unit-тесты
 ...
 *  - минимальный prompt (KS-3678) — RU и EN;
 *  - pruneUnknownSubterms (whitelist неизвестных subterm-id);
 *  - parseAndValidate (markdown-фенс, длина, типы);
 *  - postValidate (NAG-blacklist + min-length);
...
describe('ReviewCommentService', () => {
  ...
  describe('buildSystemPrompt (KS-3678 — минимальный prompt)', () => {
    const svc = new ReviewCommentService(
      makeConfigService({}),
      makeRedisStub(),
    );
    ...
    it('минимальный prompt: не содержит CRITICAL RULES / ELO-калибровки / FORBIDDEN / few-shot', () => {
      const ru = svc.buildSystemPrompt('ru', 1500);
      const en = svc.buildSystemPrompt('en', 1500);
      for (const p of [ru, en]) {
        ...
        expect(p).not.toContain('ПЛОХО');
        expect(p).not.toContain('BAD:');
        expect(p).not.toContain('Use simple terms');
        expect(p).not.toContain('Простые слова');
        expect(p).not.toContain('User ELO:');
      }
    });

    it('prompt не зависит от userElo (KS-3678 — ELO-калибровка убрана)', () => {
      expect(svc.buildSystemPrompt('ru', 800)).toBe(
        svc.buildSystemPrompt('ru', 2800),
      );
      expect(svc.buildSystemPrompt('en', 800)).toBe(
        svc.buildSystemPrompt('en', 2800),
      );
    });
  ...
  // ─────────────────────────────────────────────────────────────────
  // KS-3651 / ADR-107 rev 2 §6. pruneUnknownSubterms отбрасывает
  // unknown subterm-id с WARN перед отправкой в LLM. Логика не
  // зависит от промта и сохранена в KS-3678.
  // ─────────────────────────────────────────────────────────────────
  describe('KS-3651 pruneUnknownSubterms', () => {
    const svc = new ReviewCommentService(
      makeConfigService({}),
      makeRedisStub(),
    );
  ...
  describe('postValidate — NAG-blacklist и min-length (ADR-103 §8)', () => {
    ...
    it('postValidate активна на минимальном промте (KS-3678)', () => {
      // NAG-blacklist режется независимо от содержимого промта.
      const v = new ReviewCommentService(
        makeConfigService({}),
        makeRedisStub(),
      );
      expect(v.postValidate(['Strong move.'])).toEqual(['']);
    });
