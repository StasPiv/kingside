...
describe('PuzzleService', () => {
  ...
  describe('KS-299: no repeated attempted puzzles', () => {
    // ── KS-2562: Redis-кэш по rating-bucket'ам в getNextPuzzle ────

    describe('getNextPuzzle KS-2562 fast path', () => {
      let redis: { get: jest.Mock; set: jest.Mock };

      beforeEach(() => {
        // Inject свой redis-mock в свежий service.
        redis = {
          get: jest.fn(),
          set: jest.fn().mockResolvedValue('OK'),
        };
        service = new PuzzleService(prisma, i18n, ratingService, redis as any, mistakes);
      });

      it('cache hit: mget IDs, точечный findUnique, без $queryRawUnsafe', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        // Buckets для 1300..1700 → 1300, 1400, 1500, 1600, 1700.
        redis.get.mockImplementation(async (key: string) => {
          if (key === 'puzzle:hot:1500') return JSON.stringify(['p-hit']);
          return JSON.stringify([]);
        });
        prisma.puzzleAttempt.findMany.mockResolvedValue([]); // нет solved
        prisma.puzzle.findUnique.mockResolvedValue({
          id: 'p-hit',
          fen: 'fen',
          moves: 'e2e4',
        ...
        expect(result.id).toBe('p-hit');
        // findUnique вызван (1 heap fetch).
        expect(prisma.puzzle.findUnique).toHaveBeenCalledWith({
          where: { id: 'p-hit' },
        });
        // Прямой $queryRawUnsafe (slow path) НЕ вызван.
        expect(prisma.$queryRawUnsafe).not.toHaveBeenCalled();
        // Все 5 buckets были запрошены из cache.
        expect(redis.get).toHaveBeenCalledTimes(5);
      });

      it('cache miss → SQL fill + cache.set, потом findUnique', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        redis.get.mockResolvedValue(null); // все buckets miss
        // $queryRaw возвращает IDs (один bucket даст hit, остальные пусто).
        prisma.$queryRaw.mockImplementation(async () => [{ id: 'p-fresh' }]);
        prisma.puzzleAttempt.findMany.mockResolvedValue([]);
        prisma.puzzle.findUnique.mockResolvedValue({
          id: 'p-fresh',
          fen: 'fen',
        ...
        const result = await service.getNextPuzzle('user-1');
        expect(result.id).toBe('p-fresh');
        // 5 buckets × 1 SQL = 5 prisma.$queryRaw вызовов.
        expect(prisma.$queryRaw).toHaveBeenCalled();
        // cache.set вызван (заполнили miss'ы).
      ...
      it('фильтрует solved (login)', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        redis.get.mockResolvedValue(JSON.stringify(['p-solved', 'p-fresh']));
        prisma.puzzleAttempt.findMany.mockResolvedValue([
          { puzzleId: 'p-solved' },
        ]);
        prisma.puzzle.findUnique.mockResolvedValue({
        ...
        const result = await service.getNextPuzzle('user-1');
        expect(result.id).toBe('p-fresh'); // p-solved отфильтрован
      });

      it('excludeId исключает текущий пазл', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        redis.get.mockResolvedValue(JSON.stringify(['p-current', 'p-next']));
        prisma.puzzleAttempt.findMany.mockResolvedValue([]);
        prisma.puzzle.findUnique.mockResolvedValue({
          id: 'p-next',
          fen: 'fen',
        ...
        const result = await service.getNextPuzzle('user-1', 'p-current');
        expect(result.id).toBe('p-next');
      });

      it('themes фильтр → slow path (fast path не используется)', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        prisma.$queryRawUnsafe.mockResolvedValue([
          {
            id: 'p-slow',
        ...
        const result = await service.getNextPuzzle('user-1', undefined, {
          themes: ['mate'],
        });
        expect(result.id).toBe('p-slow');
        // Fast path: redis.get не вызывался.
        expect(redis.get).not.toHaveBeenCalled();
        // Slow path вызвал $queryRawUnsafe.
        expect(prisma.$queryRawUnsafe).toHaveBeenCalled();
      });

      it('solutionMode фильтр → slow path', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        prisma.$queryRawUnsafe.mockResolvedValue([
          {
            id: 'p-pve',
        ...
        expect(result.id).toBe('p-pve');
        expect(redis.get).not.toHaveBeenCalled();
      });

      it('fast path пустой (все cache empty + SQL fill пуст) → fallback в slow path', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        redis.get.mockResolvedValue(null);
        prisma.$queryRaw.mockResolvedValue([]); // SQL fill даёт пусто
        // Slow path тоже сначала empty → fallback к низкорейтинговому.
        ...
        const result = await service.getNextPuzzle('user-1');
        expect(result.id).toBe('p-fb');
      });

      it('cache хранит JSON-массив с TTL=3600', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        redis.get.mockResolvedValue(null);
        ...
        expect(key).toMatch(/^puzzle:hot:\d+$/);
        expect(JSON.parse(value)).toEqual(['p1', 'p2']);
        expect(mode).toBe('EX');
        expect(ttl).toBe(3600);
      });

      it('redis.get падает → graceful fallback в slow path', async () => {
        prisma.user.findUnique.mockResolvedValue({ ratingPuzzle: 1500 });
        ...
        const result = await service.getNextPuzzle('user-1');
        expect(result.id).toBe('p-recover');
        expect(prisma.$queryRawUnsafe).toHaveBeenCalled();
      });
    });
