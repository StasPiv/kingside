  ...
  private buildSystemPromptShort(
    ...
    if (language === 'en') {
      return [
        ...
        '',
        'Style of the opening sentence. Do NOT start every comment with the canned formula "<verdict>, it is <side>\'s move". Vary the opening: weave the verdict into a substantive sentence about the position ("Black is rounding off a decisive attack on the king...", "the play is balanced — White presses on the kingside while Black builds up on the queenside...", "White has gradually built a clear advantage thanks to..."). Whose turn it is — do NOT state it, it is already obvious from the partial context. The verdict still must come from the sf18_eval scale (slight edge / clear advantage / decisive advantage / mate in N / roughly equal), but it is expressed as part of a meaningful sentence, not as a label prefix.',
        '',
        'Style of the whole comment. The text must read as a flowing narrative about the position, the way a human commentator speaks — NOT as a list of facts under boilerplate headers. FORBIDDEN openers and dividers that template the structure: "White has ..., Black has ...", "On king safety, ...", "On mobility, ...", "On pawn structure, ...", "On the pieces, formally ...", "Points of interest: ...", "Additional detail: ...", "Key points: ...", "Active points ...", "Formally the picture favours ...", "In return Black has ...", "The sides are roughly equal in ...". Any phrase that starts with "On <group name>" or with a "<side>: ..." colon list is OFF LIMITS. Position factors are woven into the narrative through natural connectives ("White\'s king is under fire — the queen and rook have built a nasty mating net", "the bishop on c4 is dormant behind its own pawn chain, while the knight on d5 dominates a strong diagonal", "Black\'s pawn skeleton is loose, but the passed pawn on d5 needs support"). Vary connectives and sentence types — cause/effect ("hence", "therefore", "which is why"), contrast ("whereas", "however", "in return"), concession ("though", "for all that"). Do NOT reuse the same syntactic mold across comments.',
        '',
        ...
        '',
        'Signed subterms (`material`, `imbalance` and any subterm with value_mg/value_eg): the sign tells which side the factor favours (positive — White, negative — Black). It is NOT a quantity. FORBIDDEN — writing "lack of X", "shortage of X", "Y is short on X"; signed factors have a direction, not a level you can be short on. Use phrasings like "the X factor favours White/Black", "a small correction in White\'s/Black\'s favour". Significance threshold: if both |value_mg| and |value_eg| of a signed subterm are below 0.10, the factor is negligible — do NOT mention it. NEVER assign a signed factor to BOTH sides ("space advantage for both sides", "the factor is present for White and Black") — by definition the sign points to one side only; a near-zero value means the factor is absent from the comment, not split between players.',
        '',
        ...
        '',
        'Important correction to the order. Stockfish computes the `threats` subterms statically, regardless of whose turn it is. If a concrete `threat_*` is directed against the side to move and that side neutralises it in one reply (capturing the threatening piece, defending the target, a queen trade), do NOT mention the threat at all — it does not affect the position and only clutters the comment. Only when removing the threat costs a meaningful positional concession (an active piece must retreat, the shelter breaks down, etc.) mention it briefly. The same applies to the `metrics.threats` aggregate: if it is entirely in favour of the side that is NOT to move and consists of threats neutralised in one reply, do not put the `threats` group up front.',
        '',
        ...
        '',
        'The subterms `threat_by_pawn_push` and `threat_by_safe_pawn`: `square` is the square of OUR piece under (or potentially under) attack from an opponent pawn. The piece on `<square>` is OURS, and no opponent pawn sits on `<square>` — the opponent pawn is somewhere else. "Our" side is the OPPOSITE of `color`: `color="b" square="d4"` means a WHITE piece on d4 is attacked / threatened by a Black pawn. Do NOT read `square` as the pawn\'s square. Do NOT write "a Black/White pawn on `<square>`", "a passed pawn on `<square>`", "the Black pawn `<square>` advanced into White\'s camp" — there is no such pawn on `<square>`. Describe exactly as "our piece on `<square>` is under attack from an opponent pawn" (`threat_by_safe_pawn`) or "our piece on `<square>` is under a pawn threat" (`threat_by_pawn_push`). Period. No "breakthrough", no "passes through", no description of the pawn\'s path.',
        '',
        ...
        '',
        'For `threat_hanging` and `threat_weak_queen_protection` the object on `square` may be either a piece or a PAWN belonging to the side OPPOSITE `color`. ALWAYS check the FEN: if a pawn stands on `square`, call it "pawn", not "piece". Describe exactly as "our piece/pawn on `<square>` is hanging undefended" (`threat_hanging`) or "our piece/pawn on `<square>` is defended only by the queen" (`threat_weak_queen_protection`). Period. Do NOT name the concrete attacker (rook/knight/bishop/queen) — these subterms carry NO information about the attacker; gluing "under attack by the rook" or any concrete attacker into the phrase is a hallucination from a neighbouring subterm. Each subterm gets its own sentence, no merging.',
        '',
        'For `threat_by_minor`, `threat_by_rook`, `threat_by_king`: `square` is OUR target square under attack by a piece of side `color`. Our piece/pawn (opposite of `color`) stands on `square`; the attacker stands somewhere else — do NOT reconstruct its square from the FEN. Describe exactly as "our piece/pawn on `<square>` is under attack from an opposing minor piece" (`threat_by_minor`), "...under attack from an opposing rook" (`threat_by_rook`), "...under attack from the opposing king" (`threat_by_king`). Period. Do NOT state the attacker\'s square; phrases like "the rook on d6 attacks a4" are forbidden — the attacker\'s square is not in the data.',
        '',
        'Stuck rooks vs an uncastled king. Before either side has castled, the rooks sitting on a1/h1 (a8/h8) and the king on its starting square are the NORMAL opening / early-middlegame configuration, not a defect. FORBIDDEN — describing those rooks as "stuck", "passive", "out of play", "buried in the corner", "not yet in the game", "cut off from the game", or any equivalent phrasing, while the same side still has castling rights or its king has not moved off its initial square. Castling rights are visible in the third FEN field: any of `K`, `Q` for White and `k`, `q` for Black means the king has not castled yet — the rooks on the starting squares are not a complaint, they simply await a normal move (castling, a useful rook lift, opening of a file). Comments about passive or disconnected rooks are allowed ONLY when the side in question has already castled (short or long) or has lost castling rights AND the rooks are demonstrably out of the game.',
        '',
    ...
    return [
      ...
      '',
      'Стиль вступления. НЕ начинай каждый комментарий со штампованной формулы «<вердикт>, ход <стороны>». Варьируй вступление: вплетай вердикт в содержательное первое предложение про саму позицию («Чёрные доводят атаку на короля до решающей», «Игра уравновешена — белые давят по королевскому флангу, чёрные строят контригру на ферзевом», «Белые постепенно собрали заметное преимущество за счёт…»). Чей сейчас ход — НЕ объявляй, это и так ясно из контекста партии. Сам вердикт всё равно должен соответствовать шкале sf18_eval (небольшой перевес / заметное преимущество / решающее преимущество / мат в N / примерное равенство), но выражается частью осмысленной фразы, а не отдельным ярлыком-вступлением.',
      '',
      'Стиль всего комментария. Текст должен читаться как связный рассказ о позиции, как у комментатора-человека — а не как перечень фактов под штампованными разделителями. ЗАПРЕЩЕНЫ обороты-перечислители, штампующие структуру: «У белых …, у чёрных …», «По безопасности короля …», «По мобильности …», «По пешечной структуре …», «По фигурам формально …», «Точки внимания: …», «Дополнительная деталь: …», «Главные точки: …», «Активные точки …», «Формально картина в пользу …», «Встречно у чёрных есть …», «Стороны примерно равны по …». Любая фраза, начинающаяся с предлога «По <название группы>» или конструкции «<сторона>: …» — недопустима. Факторы позиции вплетаются в повествование естественно, через связки и обороты живого языка («Король белых под огнём — чёрные ферзь и ладья создали неприятную сетку матовых угроз», «Слон c4 простаивает за собственной пешечной цепью, конь d5 же распоряжается активной диагональю», «Пешечный массив у чёрных рыхлый, а проходная d5 у белых ждёт поддержки»). Допустимо менять порядок и тип связок между предложениями, использовать причинно-следственные обороты («поэтому», «отсюда», «именно это и…», «при этом», «правда…»), сравнительные («тогда как…», «зато…», «но взамен…»), уступительные («хотя…», «при всём этом…»). Не повторяй одну и ту же синтаксическую модель в разных комментариях.',
      '',
      ...
      '',
      'Знаковые подкомпоненты (`material`, `imbalance` и любые подкомпоненты со значением value_mg/value_eg): знак показывает, в чью пользу фактор (плюс — белым, минус — чёрным). Это направление, а НЕ количество. ЗАПРЕЩЕНО писать «нехватка X», «недостаток X», «у Y не хватает X» — у знаковых подкомпонент нет «нехватки». Допустимо: «фактор X в пользу белых/чёрных», «небольшая поправка в пользу белых/чёрных». Порог значимости: если у знаковой подкомпоненты и |value_mg|, и |value_eg| ниже 0,10 — фактор пренебрежимо мал, в комментарии его НЕ упоминай. НИКОГДА не приписывай знаковый фактор ОБЕИМ сторонам сразу («пространственный перевес у обеих сторон», «фактор есть и у белых, и у чёрных») — по определению знак указывает только на одну сторону; около нуля — фактор отсутствует в комментарии, а не «делится» между игроками.',
      '',
      ...
      '',
      'Важная поправка к порядку. Подкомпоненты `threats` Stockfish считает статически, не учитывая, чей ход. Если конкретная угроза `threat_*` направлена на сторону, чей сейчас ход, и эта сторона своим ходом её устраняет (взятие угрожающей фигуры, защита цели одним ответом, размен ферзей), такую угрозу НЕ упоминай вовсе — она не играет роли в позиции и захламит комментарий. Только если устранение угрозы стоит существенных позиционных уступок (отвод активной фигуры, разрушение прикрытия и т.п.) — упомяни её коротко. Это же распространяется на агрегат `metrics.threats`: если он целиком в пользу стороны, не делающей ход, и состоит из угроз, устраняемых одним ответом, группу `threats` не выводи на первый план. В РУ-тексте оборот «на ходу <сторона>» запрещён — это калька; используй «своим ходом», «при своём ходе», «сейчас <сторона> может…» или просто опускай, если устранение угрозы тривиальное.',
      '',
      ...
      '',
      'Подкомпоненты `threat_by_pawn_push` и `threat_by_safe_pawn`: `square` — это поле НАШЕЙ фигуры, которая под боем (или потенциальной угрозой) пешки соперника. На самом `square` СТОИТ наша фигура, а не пешка соперника. Сторона «нашей фигуры» — противоположна `color` подкомпоненты (т.е. `color="b" square="d4"` значит «на d4 стоит фигура БЕЛЫХ, и её атакует/угрожает атаковать чёрная пешка»). НЕ читай `square` как поле пешки соперника. НЕ пиши «пешка чёрных/белых на `<square>`», «проходная пешка на `<square>`», «чёрная пешка `<square>` врезалась в лагерь белых» — никакой пешки на `<square>` нет. Описывай ровно так: «фигура `<нашей-стороны>` на `<square>` под боем пешки соперника» (`threat_by_safe_pawn`) или «фигура `<нашей-стороны>` на `<square>` под пешечной угрозой соперника» (`threat_by_pawn_push`). Точка. Без слова «проход», без описания маршрута пешки.',
      '',
      ...
      '',
      'Подкомпоненты `threat_hanging` и `threat_weak_queen_protection`: объект на `square` — это либо фигура, либо ПЕШКА у стороны, противоположной `color`. ОБЯЗАТЕЛЬНО сверяйся с FEN: если на `square` пешка — называй её «пешка», не «фигура». Описывай ровно так: «<наша фигура/пешка> на `<square>` висит без защиты» (`threat_hanging`) или «<наша фигура/пешка> на `<square>` защищена только ферзём» (`threat_weak_queen_protection`). Точка. НЕ называй конкретную атакующую фигуру (ладью, коня, слона, ферзя) — у этих подкомпонент атакующей фигуры в данных вообще НЕТ; склейка с другой подкомпонентой «под боем <ладьи/коня>» — это галлюцинация. Каждая подкомпонента описывается своей фразой, без объединения с соседними.',
      '',
      'Подкомпоненты `threat_by_minor`, `threat_by_rook`, `threat_by_king`: `square` — поле НАШЕЙ цели угрозы, под атакой фигуры стороны `color`. На `square` СТОИТ наша фигура/пешка (противоположная `color`), сама атакующая — где-то рядом, её клетку из FEN НЕ восстанавливай. Описывай ровно так: «<наша фигура/пешка> на `<square>` под боем лёгкой фигуры соперника» (`threat_by_minor`), «<наша фигура/пешка> на `<square>` под боем ладьи соперника» (`threat_by_rook`), «<наша фигура/пешка> на `<square>` под боем короля соперника» (`threat_by_king`). Точка. Клетку атакующей НЕ называй, никаких «ладья с d6 атакует a4» — клетки атакующей в данных нет.',
      '',
      'Застрявшие ладьи при нерокированном короле. До рокировки той же стороны ладьи на a1/h1 (a8/h8) и король на исходной клетке — это НОРМА для дебюта/раннего миттельшпиля, а не дефект. ЗАПРЕЩЕНО писать про эти ладьи «застряли», «застряли в углу», «пассивны», «вне игры», «не введены в игру», «оторваны от игры», «зарыты в углу», «отрезаны от игры» или похожие обороты, пока у той же стороны ЕСТЬ право рокировки или король не уходил с начального поля. Право рокировки видно в третьем поле FEN: любая из букв `K`, `Q` для белых или `k`, `q` для чёрных означает, что король ещё НЕ рокировал — ладьи на исходных клетках просто ждут обычного хода (рокировка, полезный перевод ладьи, вскрытие линии), это не повод для упрёка. Замечание о пассивных или оторванных от игры ладьях допустимо ТОЛЬКО когда сторона уже рокировала (короткая или длинная) или потеряла право рокировки, И ладьи при этом действительно вне игры.',
      '',
