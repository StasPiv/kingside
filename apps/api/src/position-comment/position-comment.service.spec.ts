  ...
  describe('buildSystemPrompt (KS-3681 / ADR-108 §8.2 + KS-3686)', () => {
    ...
    it('обе версии — без преамбул в стиле CRITICAL RULES / few-shot / ELO', () => {
      ...
      for (const p of [ru, en]) {
        ...
        // KS-4112: после серии правок KS-4070 + блок JSON-формата
        // KS-3690 фактическая длина RU-инструкции — 12397, верхняя
        // граница поднята до 13000. Цель границы — сторожить
        // случайное разрастание промпта (раздувание контекста и
        // стоимости запроса), а не запрещать содержательные правки.
