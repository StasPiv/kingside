...
describe('findUndefendedAttack — KS-2227', () => {
  ...
  it('KS-2339: X-ray защита ладьёй d8 для ферзя b8 — Rc7-c8 НЕ создаёт угрозы', () => {
    // Чёрный король e8, чёрный ферзь b8, чёрная ладья d8.
    // Белая ладья c7, белый король e1. Ход Rc8 кладёт белую ладью
    // c8, и она прямо атакует ферзя b8. Но прямой attackers(b8, 'b')
    // не видит ладью d8 (между ними c8). После Rxb8 c8 пустеет —
    // Rd8 атакует b8. Это X-ray defender — ход НЕ должен считаться
    // решением. Других ходов с угрозой undefended нет (ферзь b8 —
    // единственный потенциальный target).
    const r = findUndefendedAttack(
      '1q1rk3/2R5/8/8/8/8/8/4K3 w - - 0 1',
    );
    expect(r.valid).toBe(false);
  });
});
