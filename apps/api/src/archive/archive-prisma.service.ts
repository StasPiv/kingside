...
/**
 * KS-4247 / ADR-131. Prisma-клиент для archive-БД в составе apps/api.
 * Второй PrismaClient в одном процессе рядом с основным `@kingside/db`,
 * использует `ARCHIVE_DATABASE_URL`. Пул соединений отдельный.
 *
 * Имя класса `ArchivePrismaService` — чтобы не конфликтовать с
 * основным `PrismaService` из `apps/api/src/prisma/prisma.service.ts`.
 *
