/**
 * KS-4023 / ADR-122. Smoke-тесты контроллера + DTO-валидации:
...
describe('PositionalTraceController (KS-4023)', () => {
  it('GET — кладёт Cache-Control: private, max-age=300', async () => {
    ...
    expect((dto as { gameId: string }).gameId).toBe(GAME);
  });
  ...
  it('DELETE — делегирует сервису userId из req.user.id', async () => {
    const service = {
      deleteByGame: jest.fn().mockResolvedValue(undefined),
    } as unknown as PositionalTraceService;
    const ctrl = new PositionalTraceController(service);
    await ctrl.delete(GAME, makeReqMock());
    expect(service.deleteByGame).toHaveBeenCalledWith(GAME, USER);
  });
});

// ─── DTO ────────────────────────────────────────────────────────────

describe('PositionalTraceUpsertDto validation (KS-4023)', () => {
